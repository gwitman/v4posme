...
class app_planilla_adelantos extends _BaseController {
    ...
    function edit(){ 
		 try{ 
			...
			$dataSession["head"]			= /*--inicio view*/ view('app_planilla_adelantos/edit_head',$dataView);//--finview
			$dataSession["body"]			= /*--inicio view*/ view('app_planilla_adelantos/edit_body',$dataView);//--finview
			$dataSession["script"]			= /*--inicio view*/ view('app_planilla_adelantos/edit_script',$dataView);//--finview
			$dataSession["footer"]			= "";
	...
	function delete(){
		try{ 
			...
			if($this->core_web_workflow->validateWorkflowStage("tb_transaction_master_rrhh_adelantos","statusID",$objTM->statusID,COMMAND_APLICABLE,$dataSession["user"]->companyID,$dataSession["user"]->branchID,$dataSession["role"]->roleID)){
				$query			= "CALL pr_planilla_remove_adelanto (?,?,?);";			
				$resultQuery	= $this->Bd_Model->executeRender(
					$query,
					[$objTM->companyID,$objTM->transactionID,$objTM->transactionMasterID]
				);	
			}
	...
	function updateElement($dataSession){
		try{
			...
			$objTM["reference1"] 					= /*inicio get post*/ $this->request->getPost("txtTypeID");
			$objTM["reference2"] 					= /*inicio get post*/ $this->request->getPost("txtCycleID");
			...
			if( $this->core_web_workflow->validateWorkflowStage("tb_transaction_master_rrhh_adelantos","statusID",$objTM["statusID"],COMMAND_APLICABLE,$dataSession["user"]->companyID,$dataSession["user"]->branchID,$dataSession["role"]->roleID) &&  $objTransactionMaster->statusID != $objTM["statusID"] ){
				...
				//Actualizar Planilla.
				$query			= "CALL pr_planilla_update_adelanto (?,?,?);";			
				$resultQuery	= $this->Bd_Model->executeRender(
					$query,
			...
			if($db->transStatus() !== false){
				...
				$this->response->redirect(base_url()."/".'app_planilla_adelantos/edit/companyID/'.$companyID.'/transactionID/'.$transactionID.'/transactionMasterID/'.$transactionMasterID);
			}
			else{
				$db->transRollback();						
	...
	function add(){ 
		...
		try{ 
			...
			$dataSession["notification"]	= $this->core_web_error->get_error($dataSession["user"]->userID);
			$dataSession["message"]			= $this->core_web_notification->get_message();
			$dataSession["head"]			= /*--inicio view*/ view('app_planilla_adelantos/news_head',$dataView);//--finview
			$dataSession["body"]			= /*--inicio view*/ view('app_planilla_adelantos/news_body',$dataView);//--finview
			$dataSession["script"]			= /*--inicio view*/ view('app_planilla_adelantos/news_script',$dataView);//--finview
			$dataSession["footer"]			= "";
			return view("core_masterpage/default_masterpage",$dataSession);//--finview-r
	...
	function index($dataViewID = null){	
		try{ 
			...
			$dataSession["notification"]	= $this->core_web_error->get_error($dataSession["user"]->userID);
			$dataSession["message"]			= $this->core_web_notification->get_message();
			$dataSession["head"]			= /*--inicio view*/ view('app_planilla_adelantos/list_head');//--finview
			$dataSession["footer"]			= /*--inicio view*/ view('app_planilla_adelantos/list_footer');//--finview
			$dataSession["body"]			= $dataViewRender; 
			$dataSession["script"]			= /*--inicio view*/ view('app_planilla_adelantos/list_script');//--finview
