	...
	function sales_detail_traking_subshop(){
		try{ 
			...
			if(!($viewReport && $startOn && $endOn  )){
				...
				if(!$objWarehouse)
				return helper_notificationPage("Error","<script></script>", "Bodega con la key no encontrada","","",true,"red");
	...
	function sales_detail_out_of_range()
	{
		try{ 
			...
			if(!($viewReport && $startOn && $endOn  )){
				...
				$dataSession["objListWarehouse"]		= $this->Userwarehouse_Model->getRowByUserID($companyID,$userID);
				$dataSession["message"]					= $this->core_web_notification->get_message();
				$dataSession["head"]					= /*--inicio view*/ view('app_sales_report/sales_detail_out_of_range/view_head');//--finview
				$dataSession["body"]					= /*--inicio view*/ view('app_sales_report/sales_detail_out_of_range/view_body',$dataSession);//--finview
				$dataSession["script"]					= /*--inicio view*/ view('app_sales_report/sales_detail_out_of_range/view_script');//--finview
				$dataSession["footer"]					= "";			
				return view("core_masterpage/default_report",$dataSession);//--finview-r	
			}
			else{				
				...
				$objCompany 	= $this->Company_Model->get_rowByPK($companyID);
				//Get Datos
				$query			= "CALL pr_sales_get_report_sales_detail_out_of_range(?,?,?,?,?,?,?,?,?);";				
				$objData		= $this->Bd_Model->executeRender(
					$query,
					[$companyID,$tocken,$userID,$startOn,$endOn,0,0,0,'todas']
				);
	...
	function sales_by_references()
	{
		try{ 
			...
			if(!($startOn && $endOn  )){
				...
				$dataSession["objListWarehouse"]		= $this->Userwarehouse_Model->getRowByUserID($companyID,$userID);
				$dataSession["message"]					= $this->core_web_notification->get_message();
				$dataSession["head"]					= /*--inicio view*/ view('app_sales_report/sales_by_references/view_head');//--finview
				$dataSession["body"]					= /*--inicio view*/ view('app_sales_report/sales_by_references/view_body',$dataSession);//--finview
				$dataSession["script"]					= /*--inicio view*/ view('app_sales_report/sales_by_references/view_script');//--finview
				$dataSession["footer"]					= "";			
				return view("core_masterpage/default_report",$dataSession);//--finview-r	
			}
			else{				
				...
				$query			= "CALL pr_sales_get_repor_sales_by_reference(?,?,?,?,?,?,?);";				
				$objData		= $this->Bd_Model->executeRender(
					$query,
				...
				return view("app_sales_report/sales_by_references/view_a_disemp",$objDataResult);//--finview-r
