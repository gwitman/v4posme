...
class app_invoice_survery extends _BaseController {
	...
	function index(){	
		try{
			...
			if(!$dataview["objListItem"])
			{
				$dataview["objListItem"]	= $this->Item_Model->get_rowByItemReference1And_RealStateRoomBatchServices($objListPrice->listPriceID,"");
			}
    ...
    function viewRegisterFormatoPaginaNormal80mmOpcion1(){
		try{ 
			...
			foreach($datView["objTMD"] as $detail_){
				$row = array(
					$detail_->barCode." ".$detail_->itemName." ".$detail_->reference1." ".$detail_->reference2. " ". strtolower($detail_->skuFormatoDescription)."-comand-new-row",  				
					"none",
					"none"
				);
			    array_push($detalle,$row);
