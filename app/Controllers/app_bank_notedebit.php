...
class app_bank_notedebit extends _BaseController
{
	function index($dataViewID = null)
	{
		try {
			...
			$objComponent        = $this->core_web_tools->getComponentIDBy_ComponentName("tb_transaction_master_bank_notedebit");
			if (!$objComponent)
				throw new Exception("00409 EL COMPONENTE 'tb_transaction_master_bank_notedebit' NO EXISTE...");
			...
			//Vista por defecto MOBILE
				...
				$parameter["{companyID}"]       = $this->session->get('user')->companyID;
				$dataViewData                   = $this->core_web_view->getViewByName($this->session->get('user'), $objComponent->componentID, "LISTA DE NOTAS DE DEBITO DE BANCO", CALLERID_LIST, $resultPermission, $parameter);
				$dataViewRender                 = $this->core_web_view->renderGreed($dataViewData, 'ListView', "fnTableSelectedRow");
			}
			//Vista Por Id
			else {
			...
			$dataSession["head"]                = /*--inicio view*/ view('app_bank_notedebit/list_head'); //--finview
			$dataSession["footer"]              = /*--inicio view*/ view('app_bank_notedebit/list_footer'); //--finview
			$dataSession["body"]                = $dataViewRender;
			$dataSession["script"]              = /*--inicio view*/ view('app_bank_notedebit/list_script'); //--finview
	...
	function save($mode = "")
	{
		...
		try {
			...
			if (!$this->validation->withRequest($this->request)->run()) {
				...
				$this->response->redirect(base_url() . "/" . 'app_bank_notedebit/add');
				exit;
			}
	...
	function add()
	{
		try {
			...
			$objComponentNoteDebit        = $this->core_web_tools->getComponentIDBy_ComponentName("tb_transaction_master_bank_notedebit");
			...
			$dataView["objListWorkflowStage"]                   = $this->core_web_workflow->getWorkflowInitStage("tb_transaction_master_bank_notedebit", "statusID", $companyID, $branchID, $roleID);
			...
			$dataSession["notification"]        = $this->core_web_error->get_error($dataSession["user"]->userID);
			$dataSession["message"]             = $this->core_web_notification->get_message();
			$dataSession["head"]                = /*--inicio view*/ view('app_bank_notedebit/news_head', $dataView); //--finview
			$dataSession["body"]                = /*--inicio view*/ view('app_bank_notedebit/news_body', $dataView); //--finview
			$dataSession["script"]              = /*--inicio view*/ view('app_bank_notedebit/news_script', $dataView); //--finview
			$dataSession["footer"]              = "";
			return view("core_masterpage/default_masterpage", $dataSession); //--finview-r
	...
	function edit()
	{
		try {
			...
			$dataView["objComponentNoteDebit"]	= $objComponentNoteDebit;
			$dataView["objListCurrency"]        = $this->Company_Currency_Model->getByCompany($companyID);
			$dataView["objListWorkflowStage"]   = $this->core_web_workflow->getWorkflowAllStage("tb_transaction_master_bank_notedebit", "statusID", $dataSession["user"]->companyID, $dataSession["user"]->branchID, $dataSession["role"]->roleID);

			//RENDERIZAR RESULTADO
			$dataSession["notification"]    = $this->core_web_error->get_error($dataSession["user"]->userID);
			$dataSession["message"]         = $this->core_web_notification->get_message();
			$dataSession["head"]            = view('app_bank_notedebit/edit_head', $dataView);
			$dataSession["body"]            = view('app_bank_notedebit/edit_body', $dataView);
			$dataSession["script"]          = view('app_bank_notedebit/edit_script', $dataView);
			$dataSession["footer"]          = "";
	...
	function insertElement($dataSession)
	{
		try {
			...
			$objTM["transactionNumber"]             = $this->core_web_counter->goNextNumber($companyID, $branchID, "tb_transaction_master_bank_notedebit", 0);
			$objTM["transactionID"]                 = $this->core_web_transaction->getTransactionID($companyID, "tb_transaction_master_bank_notedebit", 0);
			...
			if ($db->transStatus() !== false) {
				...
				$this->response->redirect(base_url() . "/" . 'app_bank_notedebit/edit/companyID/' . $companyID . "/transactionID/" . $objTM["transactionID"] . "/transactionMasterID/" . $transactionMasterID);
			} else {
				$db->transRollback();
	...
	function updateElement($dataSession)
	{
		try {
			...
			if (!$this->core_web_workflow->validateWorkflowStage("tb_transaction_master_bank_notedebit", "statusID", $objTM->statusID, COMMAND_EDITABLE_TOTAL, $dataSession["user"]->companyID, $dataSession["user"]->branchID, $dataSession["role"]->roleID))
				throw new Exception(NOT_WORKFLOW_EDIT);

			$db = db_connect();
			$db->transStart();

			if ($this->core_web_workflow->validateWorkflowStage("tb_transaction_master_bank_notedebit", "statusID", $objTM->statusID, COMMAND_EDITABLE, $dataSession["user"]->companyID, $dataSession["user"]->branchID, $dataSession["role"]->roleID)) {
				$objTMNew								= array();
			...
			if ($this->core_web_workflow->validateWorkflowStage("tb_transaction_master_bank_notedebit", "statusID", $objTMNew["statusID"], COMMAND_APLICABLE, $companyID, $branchID, $roleID)) {
			...
			if ($db->transStatus() !== false) {
				$db->transCommit();
				$this->core_web_notification->set_message(false, SUCCESS);
				$this->response->redirect(base_url() . "/" . 'app_bank_notedebit/edit/companyID/' . $companyID . "/transactionID/" . $transactionID . "/transactionMasterID/" . $transactionMasterID);
			} else {
				$db->transRollback();
				$this->core_web_notification->set_message(true, $this->$db->_error_message());
				$this->response->redirect(base_url() . "/" . 'app_bank_notedebit/add');
	...
	function delete()
	{
		try {
			...
			if (!$this->core_web_workflow->validateWorkflowStage("tb_transaction_master_bank_notedebit", "statusID", $objTM->statusID, COMMAND_ELIMINABLE, $dataSession["user"]->companyID, $dataSession["user"]->branchID, $dataSession["role"]->roleID))
