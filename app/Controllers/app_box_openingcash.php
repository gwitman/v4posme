...
class app_box_openingcash extends _BaseController
{
    ...
    public function edit()
    {
        try {
            ...
            if ((! $companyID || ! $transactionID || ! $transactionMasterID)) {
                $this->response->redirect(base_url() . "/" . 'app_box_openingcash/add');
            }

            //Componente de facturacion
            $objComponentTransactionShare = $this->core_web_tools->getComponentIDBy_ComponentName("tb_transaction_master_opening_cash");
            if (! $objComponentTransactionShare) {
                throw new \Exception("EL COMPONENTE 'tb_transaction_master_opening_cash' NO EXISTE...");
            }
            ...
            $dataView["objListWorkflowStage"]                = $this->core_web_workflow->getWorkflowStageByStageInit("tb_transaction_master_opening_cash", "statusID", $dataView["objTransactionMaster"]->statusID, $companyID, $branchID, $roleID);
            $dataView["objTipoMovement"]                     = $this->core_web_catalog->getCatalogAllItem("tb_transaction_master_opening_cash", "areaID", $companyID);
            $dataView["objSubTipoMovement"]                  = $this->core_web_catalog->getCatalogAllItem_Parent("tb_transaction_master_opening_cash", "priorityID", $companyID, $dataView["objTransactionMaster"]->areaID);
            ...
            $dataSession["head"]         = /*--inicio view*/view('app_box_openingcash/edit_head', $dataView);   //--finview
            $dataSession["body"]         = /*--inicio view*/view('app_box_openingcash/edit_body', $dataView);   //--finview
            $dataSession["script"]       = /*--inicio view*/view('app_box_openingcash/edit_script', $dataView); //--finview
            $dataSession["footer"]       = "";
    ...
    public function delete()
    {
        try {
			...
			$resultCmd = $this->core_web_workflow->validateWorkflowStage("tb_transaction_master_opening_cash", "statusID", $objTM->statusID, COMMAND_ELIMINABLE, $dataSession["user"]->companyID, $dataSession["user"]->branchID, $dataSession["role"]->roleID);
            if ($resultCmd == 0) {
    ...
    public function updateElement($dataSession)
    {
        try {
            ...
            $objComponentShare = $this->core_web_tools->getComponentIDBy_ComponentName("tb_transaction_master_opening_cash");
            ...
            if (! $this->core_web_workflow->validateWorkflowStage("tb_transaction_master_opening_cash", "statusID", $objTM->statusID, COMMAND_EDITABLE_TOTAL, $dataSession["user"]->companyID, $dataSession["user"]->branchID, $dataSession["role"]->roleID)) {
            ...
            if ($this->core_web_workflow->validateWorkflowStage("tb_transaction_master_opening_cash", "statusID", $objTM->statusID, COMMAND_EDITABLE, $dataSession["user"]->companyID, $dataSession["user"]->branchID, $dataSession["role"]->roleID)) {
                $objTMNew             = [];
                $objTMNew["statusID"] = /*inicio get post*/$this->request->getPost("txtStatusID");
			...
			//Si la caja esta abierta y no fui yo quie la abrio valido
				...
				$this->response->redirect(base_url() . "/" . 'app_box_openingcash/edit/companyID/' . $companyID . "/transactionID/" . $transactionID . "/transactionMasterID/" . $transactionMasterID);
				return;
			}
			//Si la caja fue abierta por mi, valido que no sea la misma transaccion
				...
				$db->transRollback();
				$this->core_web_notification->set_message(true, "Esta caja ya se encuentra abierta por: " . $cashBoxSessionUserName." cerrar la caja para abrirla nuevamente");
				$this->response->redirect(base_url() . "/" . 'app_box_openingcash/edit/companyID/' . $companyID . "/transactionID/" . $transactionID . "/transactionMasterID/" . $transactionMasterID);
				return;
			}
    ...
    public function insertElement($dataSession)
    {
        try {
            ...
            //Obtener transaccion
            $transactionID = $this->core_web_transaction->getTransactionID($dataSession["user"]->companyID, "tb_transaction_master_opening_cash", 0);
            $companyID     = $dataSession["user"]->companyID;
            ...
            $objTM["transactionNumber"]   = $this->core_web_counter->goNextNumber($dataSession["user"]->companyID, $dataSession["user"]->branchID, "tb_transaction_master_opening_cash", 0);
            ...
            if ($db->transStatus() !== false) {
                $db->transCommit();
                $this->core_web_notification->set_message(false, SUCCESS);
                $this->response->redirect(base_url() . "/" . 'app_box_openingcash/edit/companyID/' . $companyID . "/transactionID/" . $objTM["transactionID"] . "/transactionMasterID/" . $transactionMasterID);
            } else {
                $db->transRollback();
                $this->core_web_notification->set_message(true, $this->db->_error_message());
                $this->response->redirect(base_url() . "/" . 'app_box_openingcash/add');
    ...
    public function add()
    {
        ...
        try {
            ...
            $dataView["objTipoMovement"]      = $this->core_web_catalog->getCatalogAllItem("tb_transaction_master_opening_cash", "areaID", $companyID);
            $dataView["objSubTipoMovement"]   = $this->core_web_catalog->getCatalogAllItem_Parent("tb_transaction_master_opening_cash", "priorityID", $companyID, $dataView["objTipoMovement"][0]->catalogItemID);
            $dataView["objListDenomination"]  = $this->core_web_catalog->getCatalogAllItem("tb_transaction_master_denomination", "catalogItemID", $companyID);
            $dataView["objListBranch"]        = $this->Branch_Model->getByCompany($companyID);
            ...
            $dataSession["notification"] = $this->core_web_error->get_error($dataSession["user"]->userID);
            $dataSession["message"]      = $this->core_web_notification->get_message();
            $dataSession["head"]         = /*--inicio view*/view('app_box_openingcash/news_head', $dataView);   //--finview
            $dataSession["body"]         = /*--inicio view*/view('app_box_openingcash/news_body', $dataView);   //--finview
            $dataSession["script"]       = /*--inicio view*/view('app_box_openingcash/news_script', $dataView); //--finview
            $dataSession["footer"]       = "";
            return view("core_masterpage/default_masterpage", $dataSession); //--finview-r
    ...
    public function index($dataViewID = null)
    {
        try {
            ...
            $objComponent = $this->core_web_tools->getComponentIDBy_ComponentName("tb_transaction_master_opening_cash");
            if (! $objComponent) {
                throw new \Exception("00409 EL COMPONENTE 'tb_transaction_master_opening_cash' NO EXISTE...");
            }
            ...
            $dataSession["notification"] = $this->core_web_error->get_error($dataSession["user"]->userID);
            $dataSession["message"]      = $this->core_web_notification->get_message();
            $dataSession["head"]         = /*--inicio view*/view('app_box_openingcash/list_head');   //--finview
            $dataSession["footer"]       = /*--inicio view*/view('app_box_openingcash/list_footer'); //--finview
            $dataSession["body"]         = $dataViewRender;
            $dataSession["script"]       = /*--inicio view*/view('app_box_openingcash/list_script'); //--finview
    ...
    public function viewRegisterFormatoPaginaTicket()
    {
        ...
        try {
            ...
            $datView["objStage"]             = $this->core_web_workflow->getWorkflowStage("tb_transaction_master_opening_cash", "statusID", $datView["objTM"]->statusID, $companyID, $branchID, $roleID);
