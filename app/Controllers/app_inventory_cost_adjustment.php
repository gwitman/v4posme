...
use Exception;
class app_inventory_cost_adjustment extends _BaseController {
	...
	function delete(){
		try{ 
			...
			if(!$this->core_web_workflow->validateWorkflowStage("tb_transaction_master_cost_adjustment","statusID",$objTM->statusID,COMMAND_ELIMINABLE,$dataSession["user"]->companyID,$dataSession["user"]->branchID,$dataSession["role"]->roleID))
	...
	function insertElement($dataSession){
		try{
			...
			$objComponent							= $this->core_web_tools->getComponentIDBy_ComponentName("tb_transaction_master_cost_adjustment");
			if(!$objComponent)
			throw new \Exception("EL COMPONENTE 'tb_transaction_master_cost_adjustment' NO EXISTE...");
			...
			//Obtener transaccion
			$transactionID 							= $this->core_web_transaction->getTransactionID($dataSession["user"]->companyID,"tb_transaction_master_cost_adjustment",0);			
			...
			$objTM["transactionNumber"]				= $this->core_web_counter->goNextNumber($dataSession["user"]->companyID,$dataSession["user"]->branchID,"tb_transaction_master_cost_adjustment",0);
			...
			if( $this->core_web_workflow->validateWorkflowStage("tb_transaction_master_cost_adjustment","statusID",$objTM["statusID"],COMMAND_APLICABLE,$dataSession["user"]->companyID,$dataSession["user"]->branchID,$dataSession["role"]->roleID) ){
			...
			if($db->transStatus() !== false){
				...
				$this->response->redirect(base_url()."/".'app_inventory_cost_adjustment/edit/companyID/'.$companyID."/transactionID/".$objTM["transactionID"]."/transactionMasterID/".$transactionMasterID);
			}
			else{
				$db->transRollback();			
				...
				$this->response->redirect(base_url()."/".'app_inventory_cost_adjustment/add');	
			}
	...
	function insertElementMobile($dataSession, $objItems){
		...
		if($validate){
			try{
				...
				$objTM["reference3"] 					= "";
				$objTM["reference4"] 					= "";
				$objTM["statusID"] 						= $this->core_web_workflow->getWorkflowStageApplyFirst("tb_transaction_master_cost_adjustment","statusID",$companyID,$branchID,$roleID)[0]->workflowStageID;
				$objTM["amount"] 						= 0;
				$objTM["isApplied"] 					= 0;
				$objTM["journalEntryID"] 				= 0;
				$objTM["classID"] 						= NULL;
				...
				//Crear la Carpeta para almacenar los Archivos del Documento
				$path_ = PATH_FILE_OF_APP."/company_".$companyID."/component_138/component_item_".$transactionMasterID;						
				if(!file_exists ($path_)){
					mkdir($path_, 0755,true);
					chmod($path_, 0755);
				}
	...
	function updateElement($dataSession){
		try{
			...
			if(!$this->core_web_workflow->validateWorkflowStage("tb_transaction_master_cost_adjustment","statusID",$objTM->statusID,COMMAND_EDITABLE_TOTAL,$dataSession["user"]->companyID,$dataSession["user"]->branchID,$dataSession["role"]->roleID))
			...
			*****************************/
			...
			if($this->core_web_workflow->validateWorkflowStage("tb_transaction_master_cost_adjustment","statusID",$objTM->statusID,COMMAND_EDITABLE,$dataSession["user"]->companyID,$dataSession["user"]->branchID,$dataSession["role"]->roleID)){
				$objTMNew								= array();
				$objTMNew["statusID"] 					= /*inicio get post*/ $this->request->getPost("txtStatusID");						
			...
			//Aplicar el Documento?
			if( $this->core_web_workflow->validateWorkflowStage("tb_transaction_master_cost_adjustment","statusID",$objTMNew["statusID"],COMMAND_APLICABLE,$dataSession["user"]->companyID,$dataSession["user"]->branchID,$dataSession["role"]->roleID) &&  $oldStatusID != $objTMNew["statusID"] ){
			...
			*****************************/		
			if($archivoCSV == ".csv")
			{
				if($db->transStatus() !== false)
				{
					$db->transCommit();						
					$this->core_web_notification->set_message(false,SUCCESS);
					$this->response->redirect(base_url()."/".'app_inventory_cost_adjustment/edit/companyID/'.$companyID."/transactionID/".$transactionID."/transactionMasterID/".$transactionMasterID);
				}
				else
				{
					$db->transRollback();						
	...
	function edit(){
		try{ 
			...
			$objComponentInputSinPost						= $this->core_web_tools->getComponentIDBy_ComponentName("tb_transaction_master_cost_adjustment");
			...
			$datView["objListWorkflowStage"]				= $this->core_web_workflow->getWorkflowStageByStageInit("tb_transaction_master_cost_adjustment","statusID",$datView["objTM"]->statusID,$companyID,$branchID,$roleID);
			...
			$datView["objListCompanyPageSetting"]			= $this->Company_Page_Setting_Model->get_rowByKeyAndController($dataSession["company"]->type,"app_inventory_cost_adjustment");
			$datView["pantallaCompra_hiddenSummaryISC"] 	= getBahavioSession(
					$dataSession["company"]->type, 
					"app_inventory_cost_adjustment", 
			...
			$dataSession["head"]							= /*--inicio view*/ view('app_inventory_cost_adjustment/edit_head',$datView);//--finview
			$dataSession["body"]							= /*--inicio view*/ view('app_inventory_cost_adjustment/edit_body',$datView);//--finview
			$dataSession["script"]							= /*--inicio view*/ view('app_inventory_cost_adjustment/edit_script',$datView);//--finview
			$dataSession["footer"]							= "";				
	...
	function add(){
		try{ 
			...
			$roleID 							= $dataSession["role"]->roleID;
			$userID								= $dataSession["user"]->userID;
			$dataView["objListWorkflowStage"]	= $this->core_web_workflow->getWorkflowInitStage("tb_transaction_master_cost_adjustment","statusID",$companyID,$branchID,$roleID);
			...
			$dataView["objListCompanyPageSetting"]	= $this->Company_Page_Setting_Model->get_rowByKeyAndController($dataSession["company"]->type,"app_inventory_cost_adjustment");
			...
			$dataSession["notification"]		= $this->core_web_error->get_error($dataSession["user"]->userID);
			$dataSession["message"]				= $this->core_web_notification->get_message();
			$dataSession["head"]				= /*--inicio view*/ view('app_inventory_cost_adjustment/news_head',$dataView);//--finview
			$dataSession["body"]				= /*--inicio view*/ view('app_inventory_cost_adjustment/news_body',$dataView);//--finview
			$dataSession["script"]				= /*--inicio view*/ view('app_inventory_cost_adjustment/news_script',$dataView);//--finview
			$dataSession["footer"]				= "";
			return view("core_masterpage/default_masterpage",$dataSession);//--finview-r
	...
	try{
			...
			$dataSession["notification"]	= $this->core_web_error->get_error($dataSession["user"]->userID);
			$dataSession["message"]			= $this->core_web_notification->get_message();
			$dataSession["head"]			= /*--inicio view*/ view('app_inventory_cost_adjustment/list_head');//--finview
			$dataSession["footer"]			= /*--inicio view*/ view('app_inventory_cost_adjustment/list_footer');//--finview
			$dataSession["body"]			= $dataViewRender; 
			$dataSession["script"]			= /*--inicio view*/ view('app_inventory_cost_adjustment/list_script');//--finview
	...
	function viewRegisterFormatoDB(){
		try{ 
			...
			$datView["objStage"]					= $this->core_web_workflow->getWorkflowStage("tb_transaction_master_cost_adjustment","statusID",$datView["objTM"]->statusID,$companyID,$datView["objTM"]->branchID,APP_ROL_SUPERADMIN);
			...
			if($objParameterShowLinkDownload == "true")
			{
				...
				$path        = "./resource/file_company/company_".$companyID."/component_138/component_item_".$transactionMasterID."/".$fileNamePut;
				...
				echo "<a 
					href='".base_url()."/resource/file_company/company_".$companyID."/component_138/component_item_".$transactionMasterID."/".
