...
class app_box_closedcash extends _BaseController
{
    ...
    public function edit()
    {
        try {
            ...
            if ((! $companyID || ! $transactionID || ! $transactionMasterID)) {
                $this->response->redirect(base_url() . "/" . 'app_box_closedcash/add');
            }

            //Componente de facturacion
            $objComponentTransactionShare = $this->core_web_tools->getComponentIDBy_ComponentName("tb_transaction_master_closed_cash");
            if (! $objComponentTransactionShare) {
                throw new \Exception("EL COMPONENTE 'tb_transaction_master_closed_cash' NO EXISTE...");
            }
            ...
            $dataView["objListWorkflowStage"] = $this->core_web_workflow->getWorkflowStageByStageInit("tb_transaction_master_closed_cash", "statusID", $dataView["objTransactionMaster"]->statusID, $companyID, $branchID, $roleID);
            $dataView["objTipoMovement"]      = $this->core_web_catalog->getCatalogAllItem("tb_transaction_master_closed_cash", "areaID", $companyID);
            $dataView["objSubTipoMovement"]   = $this->core_web_catalog->getCatalogAllItem_Parent("tb_transaction_master_closed_cash", "priorityID", $companyID, $dataView["objTransactionMaster"]->areaID);
            ...
            $dataSession["head"]         = /*--inicio view*/view('app_box_closedcash/edit_head', $dataView);   //--finview
            $dataSession["body"]         = /*--inicio view*/view('app_box_closedcash/edit_body', $dataView);   //--finview
            $dataSession["script"]       = /*--inicio view*/view('app_box_closedcash/edit_script', $dataView); //--finview
            $dataSession["footer"]       = "";
    ...
    public function delete()
    {
        try {
			...
			$resultCmd = $this->core_web_workflow->validateWorkflowStage("tb_transaction_master_closed_cash", "statusID", $objTM->statusID, COMMAND_ELIMINABLE, $dataSession["user"]->companyID, $dataSession["user"]->branchID, $dataSession["role"]->roleID);
            if ($resultCmd == 0) 
			{
    ...
    public function updateElement($dataSession)
    {
        try {
            ...
            $objComponentShare = $this->core_web_tools->getComponentIDBy_ComponentName("tb_transaction_master_closed_cash");
            ...
            if (! $this->core_web_workflow->validateWorkflowStage("tb_transaction_master_closed_cash", "statusID", $objTM->statusID, COMMAND_EDITABLE_TOTAL, $dataSession["user"]->companyID, $dataSession["user"]->branchID, $dataSession["role"]->roleID)) {
            ...
            if (
                ...
                (
                    "tb_transaction_master_closed_cash",
                    "statusID",
                    $objTMNew["statusID"],
                    COMMAND_APLICABLE,
            ...
            if ($this->core_web_workflow->validateWorkflowStage("tb_transaction_master_closed_cash", "statusID", $objTM->statusID, COMMAND_EDITABLE, $dataSession["user"]->companyID, $dataSession["user"]->branchID, $dataSession["role"]->roleID)) {
                $objTMNew             = [];
                $objTMNew["statusID"] = /*inicio get post*/$this->request->getPost("txtStatusID");
    ...
    public function insertElement($dataSession)
    {
        try {
            ...
            //Obtener transaccion
            $transactionID = $this->core_web_transaction->getTransactionID($dataSession["user"]->companyID, "tb_transaction_master_closed_cash", 0);
            $companyID     = $dataSession["user"]->companyID;
            ...
            $objTM["transactionNumber"]   = $this->core_web_counter->goNextNumber($dataSession["user"]->companyID, $dataSession["user"]->branchID, "tb_transaction_master_closed_cash", 0);
            ...
            if ($db->transStatus() !== false) {
                $db->transCommit();
                $this->core_web_notification->set_message(false, SUCCESS);
                $this->response->redirect(base_url() . "/" . 'app_box_closedcash/edit/companyID/' . $companyID . "/transactionID/" . $objTM["transactionID"] . "/transactionMasterID/" . $transactionMasterID);
            } else {
                $db->transRollback();
    ...
    public function add()
    {
        ...
        try {
            ...
            $dataView["objListWorkflowStage"] = $this->core_web_workflow->getWorkflowInitStage("tb_transaction_master_closed_cash", "statusID", $companyID, $branchID, $roleID);
            $dataView["objTipoMovement"]      = $this->core_web_catalog->getCatalogAllItem("tb_transaction_master_closed_cash", "areaID", $companyID);
            $dataView["objSubTipoMovement"]   = $this->core_web_catalog->getCatalogAllItem_Parent("tb_transaction_master_closed_cash", "priorityID", $companyID, $dataView["objTipoMovement"][0]->catalogItemID);
            $dataView["objListDenomination"]  = $this->core_web_catalog->getCatalogAllItem("tb_transaction_master_denomination", "catalogItemID", $companyID);
            $dataView["objListBranch"]        = $this->Branch_Model->getByCompany($companyID);
            ...
            $dataSession["notification"] = $this->core_web_error->get_error($dataSession["user"]->userID);
            $dataSession["message"]      = $this->core_web_notification->get_message();
            $dataSession["head"]         = /*--inicio view*/view('app_box_closedcash/news_head', $dataView);   //--finview
            $dataSession["body"]         = /*--inicio view*/view('app_box_closedcash/news_body', $dataView);   //--finview
            $dataSession["script"]       = /*--inicio view*/view('app_box_closedcash/news_script', $dataView); //--finview
            $dataSession["footer"]       = "";
            return view("core_masterpage/default_masterpage", $dataSession); //--finview-r
    ...
    public function index($dataViewID = null)
    {
        try {
            ...
            $objComponent = $this->core_web_tools->getComponentIDBy_ComponentName("tb_transaction_master_closed_cash");
            if (! $objComponent) {
                throw new \Exception("00409 EL COMPONENTE 'tb_transaction_master_closed_cash' NO EXISTE...");
            }
            ...
            $dataSession["notification"] = $this->core_web_error->get_error($dataSession["user"]->userID);
            $dataSession["message"]      = $this->core_web_notification->get_message();
            $dataSession["head"]         = /*--inicio view*/view('app_box_closedcash/list_head');   //--finview
            $dataSession["footer"]       = /*--inicio view*/view('app_box_closedcash/list_footer'); //--finview
            $dataSession["body"]         = $dataViewRender;
            $dataSession["script"]       = /*--inicio view*/view('app_box_closedcash/list_script'); //--finview
    ...
    public function searchTransactionMaster()
    {
        try {
            //AUTENTICADO
            if (! $this->core_web_authentication->isAuthenticated()) {
    ...
    public function viewRegisterFormatoPaginaTicket()
    {
        ...
        try {
            ...
            $datView["objStage"]             = $this->core_web_workflow->getWorkflowStage("tb_transaction_master_closed_cash", "statusID", $datView["objTM"]->statusID, $companyID, $branchID, $roleID);
    ...
    //factura en imppresora de ticket de 80mm
    public function viewRegisterFormatoPaginaTicketColirio()
    {
        //Factura en Impresora Termica
        //O impresora de ticket, con ancho de 3.2 pulgadas
        //O equivalente a 8 centimetro
        ...
        try {
            ...
            //Generar Reporte
            $html = helper_reporte80mmTransactionMasterInputOutPutCashColirio(
                "SALIDA DE CAJA",
                $objCompany,
                $objParameter,
                $datView["objTM"],
            ...
            //visualizar
            $this->dompdf->stream("file_" . date('YmdHis'), ['Attachment' => ! $objParameterShowLinkDownload]);
    ...
    //factura en imppresora de ticket de 80mm
    public function viewRegisterFormatoPaginaTicketTermica()
    {
        //Factura en Impresora Termica
        //O impresora de ticket, con ancho de 3.2 pulgadas
        //O equivalente a 8 centimetro
    ...
    //factura en imppresora de ticket de 80mm
    public function viewPrinterDirect80mmShareRustikGrill()
    {
        //Factura en Impresora Termica
        //O impresora de ticket, con ancho de 3.2 pulgadas
        //O equivalente a 8 centimetro
    ...
    //factura en imppresora de ticket de 80mm
    public function viewPrinterDirect80mmComidaChinaMijo()
    {
        //Factura en Impresora Termica
        //O impresora de ticket, con ancho de 3.2 pulgadas
        //O equivalente a 8 centimetro
    ...
    public function viewRegisterFormatoA4Globalpro()
    {
        try {
            ...
            //Generar Reporte
            $html = helper_reporteA4mmTransactionMasterOutputCashGlobalPro(
                "SALIDA DE CAJA",
                $objCompany,
                $objParameter,
                $datView["objTM"],
