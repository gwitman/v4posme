...
class app_cxp_notecredit extends _BaseController
{
	...
	function index($dataViewID = null)
	{
		try {
			...
			$objComponent        = $this->core_web_tools->getComponentIDBy_ComponentName("tb_transaction_master_cxp_notacredito");
			if (!$objComponent)
			...
			//Vista por defecto MOBILE
				...
				$parameter["{companyID}"]       = $this->session->get('user')->companyID;
				$dataViewData                   = $this->core_web_view->getViewByName($this->session->get('user'), $objComponent->componentID, "LISTA DE NOTAS DE CREDITO DE PROVEEDORES", CALLERID_LIST, $resultPermission, $parameter);
				$dataViewRender                 = $this->core_web_view->renderGreed($dataViewData, 'ListView', "fnTableSelectedRow");
			}
			//Vista Por Id
			else {
			...
			$dataSession["head"]                = /*--inicio view*/ view('app_cxp_notecredit/list_head'); //--finview
			$dataSession["footer"]              = /*--inicio view*/ view('app_cxp_notecredit/list_footer'); //--finview
			$dataSession["body"]                = $dataViewRender;
			$dataSession["script"]              = /*--inicio view*/ view('app_cxp_notecredit/list_script'); //--finview
	...
	function add()
	{
		...
		try {
			...
			$dataView["objListWorkflowStage"]				= $this->core_web_workflow->getWorkflowInitStage("tb_transaction_master_cxp_notacredito", "statusID", $companyID, $branchID, $roleID);
	...
	function edit()
	{
		try {
			...
			$dataView["objCustomer"]            			= $this->Provider_Model->get_rowByPK($companyID, $branchID, $dataView["objTM"]->entityID);
	...
	function updateElement($dataSession)
	{
		try {
			...
			$objC								   	= $this->Provider_Model->get_rowByPK($companyID, $branchID, $entityID);
			...
			if ($this->core_web_workflow->validateWorkflowStage("tb_transaction_master_cxp_notacredito", "statusID", $objTMNew["statusID"], COMMAND_APLICABLE, $companyID, $branchID, $roleID)) {
				...
				$this->Transaction_Master_Model->update_app_posme($companyID, $transactionID, $transactionMasterID, $objTMNew);
				$this->Provider_Model->update_app_posme($companyID, $branchID, $entityID, $objCNew);
			}
	...
	function delete()
	{
		try {
			...
			if (!$this->core_web_workflow->validateWorkflowStage("tb_transaction_master_cxp_notacredito", "statusID", $objTM->statusID, COMMAND_ELIMINABLE, $dataSession["user"]->companyID, $dataSession["user"]->branchID, $dataSession["role"]->roleID))
	...
	function viewPrinterFormatoA4()
	{
		try {
			...
			$objTM						= $this->Transaction_Master_Model->get_rowByPk($companyID, $transactionID, $transactionMasterID);
			$objCustomer                = $this->Provider_Model->get_rowByPK($companyID, $branchID, $objTM->entityID);
			...
			$objNoteEntity["number"]	= $objCustomer->providerNumber;
