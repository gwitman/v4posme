...
class app_sales_loyalty extends _BaseController
{
	function index($dataViewID = null)
	{
		try {
			...
			$objComponent        = $this->core_web_tools->getComponentIDBy_ComponentName("tb_transaction_master_sales_loyalty");
			if (!$objComponent)
				throw new Exception("00409 EL COMPONENTE 'tb_transaction_master_sales_loyalty' NO EXISTE...");
			...
			//Vista por defecto MOBILE
				...
				$parameter["{companyID}"]       = $this->session->get('user')->companyID;
				$dataViewData                   = $this->core_web_view->getViewByName($this->session->get('user'), $objComponent->componentID, "LISTA DE TARJETAS DE FIDELIDAD", CALLERID_LIST, $resultPermission, $parameter);
				$dataViewRender                 = $this->core_web_view->renderGreed($dataViewData, 'ListView', "fnTableSelectedRow");
			}
			//Vista Por Id
			else {
			...
			$dataSession["head"]                = /*--inicio view*/ view('app_sales_loyalty/list_head'); //--finview
			$dataSession["footer"]              = /*--inicio view*/ view('app_sales_loyalty/list_footer'); //--finview
			$dataSession["body"]                = $dataViewRender;
			$dataSession["script"]              = /*--inicio view*/ view('app_sales_loyalty/list_script'); //--finview
	...
	function save($mode = "")
	{
		...
		try {
			...
			//Validar Formulario						
			$this->validation->setRule("txtNumberPhone", "Telefono", "required");
			$this->validation->setRule("txtAmount", "Meta", "required");
			...
			if (!$this->validation->withRequest($this->request)->run()) {
				...
				$this->response->redirect(base_url() . "/" . 'app_sales_loyalty/add');
				exit;
			}
	...
	function add()
	{
		try {
			...
			$objComponentSalesLoyalty        = $this->core_web_tools->getComponentIDBy_ComponentName("tb_transaction_master_sales_loyalty");
			if (!$objComponentSalesLoyalty)
				throw new Exception("00409 EL COMPONENTE 'tb_transaction_master_sales_loyalty' NO EXISTE...");
			...
			$dataView["objComponentSalesLoyalty"]               = $objComponentSalesLoyalty;			
			$dataView["objListWorkflowStage"]                   = $this->core_web_workflow->getWorkflowInitStage("tb_transaction_master_sales_loyalty", "statusID", $companyID, $branchID, $roleID);
			
			//Renderizar Resultado 
			$dataSession["notification"]        = $this->core_web_error->get_error($dataSession["user"]->userID);
			$dataSession["message"]             = $this->core_web_notification->get_message();
			$dataSession["head"]                = /*--inicio view*/ view('app_sales_loyalty/news_head', $dataView); //--finview
			$dataSession["body"]                = /*--inicio view*/ view('app_sales_loyalty/news_body', $dataView); //--finview
			$dataSession["script"]              = /*--inicio view*/ view('app_sales_loyalty/news_script', $dataView); //--finview
			$dataSession["footer"]              = "";
			return view("core_masterpage/default_masterpage", $dataSession); //--finview-r
	...
	function edit()
	{
		try {
			...
			$dataView["objComponentSalesLoyalty"]	= $objComponentSalesLoyalty;
			$dataView["objListWorkflowStage"]   = $this->core_web_workflow->getWorkflowAllStage("tb_transaction_master_sales_loyalty", "statusID", $dataSession["user"]->companyID, $dataSession["user"]->branchID, $dataSession["role"]->roleID);

			//RENDERIZAR RESULTADO
			$dataSession["notification"]    = $this->core_web_error->get_error($dataSession["user"]->userID);
			$dataSession["message"]         = $this->core_web_notification->get_message();
			$dataSession["head"]            = view('app_sales_loyalty/edit_head', $dataView);
			$dataSession["body"]            = view('app_sales_loyalty/edit_body', $dataView);
			$dataSession["script"]          = view('app_sales_loyalty/edit_script', $dataView);
			$dataSession["footer"]          = "";
	...
	function insertElement($dataSession)
	{
		try {
			...
			$objTM["transactionNumber"]             = $this->core_web_counter->goNextNumber($companyID, $branchID, "tb_transaction_master_sales_loyalty", 0);
			$objTM["transactionID"]                 = $this->core_web_transaction->getTransactionID($companyID, "tb_transaction_master_sales_loyalty", 0);
			...
			$objTM["componentID"]                   = $objComponentSalesLoyalty->componentID;
			...
			$objTM["tax1"]                        	= /*inicio get post*/ $this->request->getPost("txtTax1");
			...
			if (!file_exists(PATH_FILE_OF_APP . "/company_" . $companyID . "/component_" . $objComponentSalesLoyalty->componentID . "/component_item_" . $transactionMasterID)) {
				mkdir(PATH_FILE_OF_APP . "/company_" . $companyID . "/component_" . $objComponentSalesLoyalty->componentID . "/component_item_" . $transactionMasterID, 0700, true);
			}

			if ($db->transStatus() !== false) {
				$db->transCommit();
				$this->core_web_notification->set_message(false, SUCCESS);
				$this->response->redirect(base_url() . "/" . 'app_sales_loyalty/edit/companyID/' . $companyID . "/transactionID/" . $objTM["transactionID"] . "/transactionMasterID/" . $transactionMasterID);
			} else {
				$db->transRollback();
	...
	function updateElement($dataSession)
	{
		try {
			...
			$objTMNew["tax1"]                       = /*inicio get post*/ $this->request->getPost("txtTax1");
			...
			if (!$this->core_web_workflow->validateWorkflowStage("tb_transaction_master_sales_loyalty", "statusID", $objTM->statusID, COMMAND_EDITABLE_TOTAL, $dataSession["user"]->companyID, $dataSession["user"]->branchID, $dataSession["role"]->roleID))
				throw new Exception(NOT_WORKFLOW_EDIT);

			$db = db_connect();
			$db->transStart();

			if ($this->core_web_workflow->validateWorkflowStage("tb_transaction_master_sales_loyalty", "statusID", $objTM->statusID, COMMAND_EDITABLE, $dataSession["user"]->companyID, $dataSession["user"]->branchID, $dataSession["role"]->roleID)) {
				$objTMNew								= array();
			...
			if ($db->transStatus() !== false) {
				$db->transCommit();
				$this->core_web_notification->set_message(false, SUCCESS);
				$this->response->redirect(base_url() . "/" . 'app_sales_loyalty/edit/companyID/' . $companyID . "/transactionID/" . $transactionID . "/transactionMasterID/" . $transactionMasterID);
			} else {
				$db->transRollback();
				$this->core_web_notification->set_message(true, $this->$db->_error_message());
				$this->response->redirect(base_url() . "/" . 'app_sales_loyalty/add');
	...
	function delete()
	{
		try {
			...
			if (!$this->core_web_workflow->validateWorkflowStage("tb_transaction_master_sales_loyalty", "statusID", $objTM->statusID, COMMAND_ELIMINABLE, $dataSession["user"]->companyID, $dataSession["user"]->branchID, $dataSession["role"]->roleID))
	...
	function viewPrinterFormatoA4()
	{
		try {
			...
			//Generar Reporte
			$html = helper_reporteA4TarjetaFidelidad(
				$objCompany,
				$objParameterLogo,
				$objTM,
			...
			if ($objParameterShowLinkDownload == "true") {
				...
				$path        = "./resource/file_company/company_" . $companyID . "/component_126/component_item_" . $transactionMasterID . "/" . $fileNamePut;
				...
				echo "<a 
					href='" . base_url() . "/resource/file_company/company_" . $companyID . "/component_126/component_item_" . $transactionMasterID . "/" .
