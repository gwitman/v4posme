...
class core_dashboards extends _BaseController {
    ...
    function index(){
        try{
            ...
            {
                $dataSession					= $this->getIndexElektro($dataSession);
                $dataSession["body"]			= /*--inicio view*/ view('core_dasboard/dashboards_default_elektro',$dataSession);//--finview
            }
			else if($objCompany->type == "agronic")
            {
                $dataSession					= $this->getIndexAgronic($dataSession);
                $dataSession["body"]			= /*--inicio view*/ view('core_dasboard/dashboards_default_agronic',$dataSession);//--finview
            }
            else
            {
    ...
    function getIndexElektro($dataSession)
    {
        $firstDateYear					= helper_PrimerDiaDelYear();
        $lastDateYear					= helper_UltimoDiaDelMes();
        $firstDate						= helper_PrimerDiaDelMes();
        ...
        $objFirstDate->setTime(0, 0, 0);
        $objListVentaContadoMensuales 	= array();
		$objListVentaContadoMensuales 	= $this->Transaction_Master_Detail_Model->Default_Ventas_De_Contado_Mensuales_ElektroClima($dataSession["user"]->companyID, $objFirstDate->format("Y-m-d"),$objLastDate->format("Y-m-d") );
	...
	function getIndexAgronic($dataSession)
    {
        $firstDateYear					= helper_PrimerDiaDelYear();
        $lastDateYear					= helper_UltimoDiaDelMes();
        $firstDate						= helper_PrimerDiaDelMes();
        ...
        //Obtener Ventas de Contado Mensuales (Año actual)
        $objFirstYearDate 				= \DateTime::createFromFormat('Y-m-d', $firstDateYear);
        $objFirstYearDate->setTime(0, 0, 0);
        $objFirstDate 					= \DateTime::createFromFormat('Y-m-d', $firstDate);
        $objFirstDate->setTime(0, 0, 0);
        ...
        //Obtener Ventas de Contado del Año Anterior (todo el año pasado)
        $prevYearStart 		= (intval(date('Y')) - 1) . '-01-01';
        $prevYearEnd   		= (intval(date('Y')) - 1) . '-12-31';
        $objListVentaContadoAnnioAnterior = array();
        $objListVentaContadoAnnioAnterior = $this->Transaction_Master_Detail_Model->Default_Ventas_De_Credito_Mes_Actual($dataSession["user"]->companyID, $prevYearStart,$prevYearEnd);
        ...
        $dataSession["objListVentasContadoMesActual"]		= $objListVentasContadoMesActual;
        $dataSession["objListVentaContadoMensuales"]		= $objListVentaContadoMensuales;
        $dataSession["objListVentaContadoAnnioAnterior"]	= $objListVentaContadoAnnioAnterior;
        return $dataSession;
    }
}
?>