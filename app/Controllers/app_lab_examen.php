    ...
    function edit(){
        try{
			...
			if($transactionMasterID ==  0 && $comandoID == "save"){
				...
				$objTM["reference1"] 					= "reference3= catalogItemID= catalog de examen "; 
				$objTM["reference2"] 					= "";
			...
			if ( $comandoID == "save" && $statusNewEditableParcial == false ){
				...
				$objTxtCatalogItemID				= /*inicio get post*/ $this->request->getPost("txtCatalogItemID");//--fin peticion get o post
				...
				if( isset($objTMOld))
				if(	$transactionMasterID > 0 && $objTMOld->reference3 == $objTM["reference3"] )
					$ingresarDetalle = true;
            ...
            $dataView["objTransactionMasterDetail"]				= $this->Transaction_Master_Detail_Model->get_rowByTransactionAndComponent($companyID,$transactionID,$transactionMasterID,$objComponentCatalog->componentID);           
            ...
            $dataView["objListExamenes"]			= $this->core_web_catalog->getCatalogAllItem("tb_transaction_master_examen_lab","type",$companyID);
			
			//Obtener examen por defecto
			$examenId 				= 0;			
			if($transactionMasterID == 0){
				$examenId = $dataView["objListExamenes"][0]->catalogItemID;
			}
			
			
			$dataView["objListExamenesParametros"]	= $this->core_web_catalog->getCatalogAllItem_Parent("tb_transaction_master_examen_lab","parameterExamen",$companyID,helper_RequestGetValueObjet($dataView["objTransactionMaster"],"reference3",$examenId));
	...
	function viewRegisterFormatoPaginaNormalA4LabGenerico(){
		try{ 
			...
			$row = array(
				...
				"style_row_data"		=>"ttext-align:left;width:auto",
				"colspan_row_data"		=>'1',
				"prefix_row_data"		=>'',
			...
			$row = array(
				...
				"style_row_data"		=>"text-align:left;width:10px",
				"colspan_row_data"		=>'1',
				"prefix_row_data"		=>'',
				"nueva_fila_row_data"	=>0
			);
			...
			$row = array(
				...
				"style_row_data"		=>"text-align:right;width:220px",
				"colspan_row_data"		=>'1',
				"prefix_row_data"		=>'',
				"nueva_fila_row_data"	=>0
			);
