...
class app_purchase_pedidos extends _BaseController {
    ...
    function edit(){ 
		 try{ 
			...
			{ 
				$this->response->redirect(base_url()."/".'app_purchase_pedidos/add');	
			} 		
			...
			$objComponentTransactionShare	= $this->core_web_tools->getComponentIDBy_ComponentName("tb_transaction_master_workshop_pedido");
			if(!$objComponentTransactionShare)
			throw new \Exception("EL COMPONENTE 'tb_transaction_master_workshop_pedido' NO EXISTE...");
			...
			$dataView["objListPiezas"]			= $this->core_web_catalog->getCatalogAllItem("tb_transaction_master_workshop_pedido","areaID",$companyID);
			$dataView["objListEnvios"]			= $this->core_web_catalog->getCatalogAllItem("tb_transaction_master_workshop_pedido","priorityID",$companyID);
			$dataView["objListPedidoStatus"]	= $this->core_web_catalog->getCatalogAllItem("tb_transaction_master_workshop_pedido","zoneID",$companyID);
			$dataView["objListArchivos"]		= $this->core_web_catalog->getCatalogAllItem("tb_transaction_master_workshop_pedido","mesaID",$companyID);
			
			$objParameterUrlPrinter 				= $this->core_web_parameter->getParameter("WORKSHOW_URL_PRINTER_PEDIDO",$companyID);
			...
			$dataSession["head"]			= /*--inicio view*/ view('app_purchase_pedidos/edit_head',$dataView);//--finview
			$dataSession["body"]			= /*--inicio view*/ view('app_purchase_pedidos/edit_body',$dataView);//--finview
			$dataSession["script"]			= /*--inicio view*/ view('app_purchase_pedidos/edit_script',$dataView);//--finview
			$dataSession["footer"]			= "";
	...
	function delete(){
		try{ 
			...
			if( $this->core_web_workflow->validateWorkflowStage("tb_transaction_master_workshop_pedido","statusID",$objTM->statusID,COMMAND_ELIMINABLE,$dataSession["user"]->companyID,$dataSession["user"]->branchID,$dataSession["role"]->roleID))
	...
	function updateElement($dataSession){
		try{
			...
			$objComponentShare			= $this->core_web_tools->getComponentIDBy_ComponentName("tb_transaction_master_workshop_pedido");
			...
			if(!$this->core_web_workflow->validateWorkflowStage("tb_transaction_master_workshop_pedido","statusID",$objTM->statusID,COMMAND_EDITABLE_TOTAL,$dataSession["user"]->companyID,$dataSession["user"]->branchID,$dataSession["role"]->roleID))
			...
			if($this->core_web_workflow->validateWorkflowStage("tb_transaction_master_workshop_pedido","statusID",$objTM->statusID,COMMAND_EDITABLE,$dataSession["user"]->companyID,$dataSession["user"]->branchID,$dataSession["role"]->roleID))
			{
	...
	function insertElement($dataSession){
		try{
			...
			if($db->transStatus() !== false){
				$db->transCommit();						
				$this->core_web_notification->set_message(false,SUCCESS);
				$this->response->redirect(base_url()."/".'app_purchase_pedidos/edit/companyID/'.$companyID."/transactionID/".$objTM["transactionID"]."/transactionMasterID/".$transactionMasterID);
			}
			else{
				$db->transRollback();						
	...
	function add(){ 
		...
		try{ 
			...
			$dataSession["notification"]	= $this->core_web_error->get_error($dataSession["user"]->userID);
			$dataSession["message"]			= $this->core_web_notification->get_message();
			$dataSession["head"]			= /*--inicio view*/ view('app_purchase_pedidos/news_head',$dataView);//--finview
			$dataSession["body"]			= /*--inicio view*/ view('app_purchase_pedidos/news_body',$dataView);//--finview
			$dataSession["script"]			= /*--inicio view*/ view('app_purchase_pedidos/news_script',$dataView);//--finview
			$dataSession["footer"]			= "";
			return view("core_masterpage/default_masterpage",$dataSession);//--finview-r
	...
	function index($dataViewID = null){	
		try{ 
			...
			$objComponent		= $this->core_web_tools->getComponentIDBy_ComponentName("tb_transaction_master_workshop_pedido");
			if(!$objComponent)
			throw new \Exception("00409 EL COMPONENTE 'tb_transaction_master_workshop_pedido' NO EXISTE...");
	...
	function viewPrinterFormatoA4Output(){
		try{ 
			...
			//Generar Reporte
			$html = helper_reporteA4mmTransactionMasterPedidosOutputGlobalPro(
			    "TALLER",
			    $objCompany,
			    $objParameter,
			    $datView["objTM"],
	...
	function viewPrinterFormatoA4Stiker(){
		try{ 
			...
			$datView["objCurrency"]					= $this->Currency_Model->get_rowByPK($datView["objTM"]->currencyID);
			$datView["tipoCambio"]					= round($datView["objTM"]->exchangeRate + $this->core_web_parameter->getParameter("ACCOUNTING_EXCHANGE_SALE",$companyID)->value,2);
			$datView["objStage"]					= $this->core_web_workflow->getWorkflowStage("tb_transaction_master_workshop_pedido","statusID",$datView["objTM"]->statusID,$companyID,$datView["objTM"]->branchID,APP_ROL_SUPERADMIN);
			...
			$dataView["objItemMarca"]			= $this->core_web_catalog->getCatalogItem("tb_transaction_master_workshop_pedido","zoneID",$companyID,$dataView["objTransactionMasterInfo"]->zoneID);			
			$dataView["objListArchivos"]		= $this->core_web_catalog->getCatalogAllItem("tb_transaction_master_workshop_pedido","mesaID",$companyID);
