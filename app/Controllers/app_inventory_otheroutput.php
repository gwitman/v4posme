...
class app_inventory_otheroutput extends _BaseController
{
	...
	function insertElementMobile($dataSession, $transactionMaster, $transactionMasterDetails)
	{
		try {
			...
			//Obtener el Componente de Transacciones Other Output to Inventory
			$objComponent							= $this->core_web_tools->getComponentIDBy_ComponentName("tb_transaction_master_otheroutput");
			if (!$objComponent)
				throw new \Exception("EL COMPONENTE 'tb_transaction_master_otheroutput' NO EXISTE...");
			...
			$objTM["currencyID2"]					= $objTM["currencyID"];
			$objTM["exchangeRate"]					= 1;
			$objTM["reference1"] 					= "";
			$objTM["reference2"] 					= "";
			$objTM["reference3"] 					= "";
			$objTM["reference4"] 					= "";
	...
	function insertElementMobileByItems($dataSession, $items)
	{
		try {
			$validate = false;
