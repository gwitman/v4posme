...
class app_box_canceldocument extends _BaseController {
    ...
    function edit(){ 
		 try{ 
			...
			{ 
				$this->response->redirect(base_url()."/".'app_box_canceldocument/add');	
			} 		
			...
			$objComponentTransactionCancelInvoice	= $this->core_web_tools->getComponentIDBy_ComponentName("tb_transaction_master_cancel_invoice");
			if(!$objComponentTransactionCancelInvoice)
			...
			$dataView["objComponentTransactionCancelInvoice"]	= $objComponentTransactionCancelInvoice;
			...
			$dataView["objListWorkflowStage"]	= $this->core_web_workflow->getWorkflowStageByStageInit("tb_transaction_master_cancel_invoice","statusID",$dataView["objTransactionMaster"]->statusID,$companyID,$branchID,$roleID);
			...
			$dataSession["head"]			= /*--inicio view*/ view('app_box_canceldocument/edit_head',$dataView);//--finview
			$dataSession["body"]			= /*--inicio view*/ view('app_box_canceldocument/edit_body',$dataView);//--finview
			$dataSession["script"]			= /*--inicio view*/ view('app_box_canceldocument/edit_script',$dataView);//--finview
			$dataSession["footer"]			= "";
	...
	function delete(){
		try{ 
			...
			if( $this->core_web_workflow->validateWorkflowStage("tb_transaction_master_cancel_invoice","statusID",$objTM->statusID,COMMAND_ELIMINABLE,$dataSession["user"]->companyID,$dataSession["user"]->branchID,$dataSession["role"]->roleID))
	...
	function updateElement($dataSession){
		try{
			...
			if(!$this->core_web_workflow->validateWorkflowStage("tb_transaction_master_cancel_invoice","statusID",$objTM->statusID,COMMAND_EDITABLE_TOTAL,$dataSession["user"]->companyID,$dataSession["user"]->branchID,$dataSession["role"]->roleID))
			...
			if($this->core_web_workflow->validateWorkflowStage("tb_transaction_master_cancel_invoice","statusID",$objTM->statusID,COMMAND_EDITABLE,$dataSession["user"]->companyID,$dataSession["user"]->branchID,$dataSession["role"]->roleID)){
				$objTMNew								= array();
				$objTMNew["statusID"] 					= /*inicio get post*/ $this->request->getPost("txtStatusID");						
			...
			if( $this->core_web_workflow->validateWorkflowStage("tb_transaction_master_cancel_invoice","statusID",$objTMNew["statusID"],COMMAND_APLICABLE,$dataSession["user"]->companyID,$dataSession["user"]->branchID,$dataSession["role"]->roleID) &&  $oldStatusID != $objTMNew["statusID"] ){
				...
				foreach($objListTMD as $key => $objTMD){
					...
					$this->core_web_amortization->cancelDocument($companyID,$objTMD->componentItemID,$objTMD->amount);					
				...
				//Crear Conceptos.
				$this->core_web_concept->cancelinvoice($companyID,$transactionID,$transactionMasterID);
			...
			if($db->transStatus() !== false){
				...
				$this->response->redirect(base_url()."/".'app_box_canceldocument/edit/companyID/'.$companyID."/transactionID/".$transactionID."/transactionMasterID/".$transactionMasterID);
			}
			else{
				$db->transRollback();						
	...
	function insertElement($dataSession){
		try{
			...
			//Obtener transaccion
			$transactionID 							= $this->core_web_transaction->getTransactionID($dataSession["user"]->companyID,"tb_transaction_master_cancel_invoice",0);
			$companyID 								= $dataSession["user"]->companyID;
			...
			$objTM["transactionNumber"]				= $this->core_web_counter->goNextNumber($dataSession["user"]->companyID,$dataSession["user"]->branchID,"tb_transaction_master_cancel_invoice",0);
			...
			$objTM["componentID"] 					= $objComponentCancelInvoice->componentID;
			...
			if($db->transStatus() !== false){
				$db->transCommit();						
				$this->core_web_notification->set_message(false,SUCCESS);
				$this->response->redirect(base_url()."/".'app_box_canceldocument/edit/companyID/'.$companyID."/transactionID/".$objTM["transactionID"]."/transactionMasterID/".$transactionMasterID);
			}
			else{
				$db->transRollback();						
				$this->core_web_notification->set_message(true,$this->db->_error_message());
	...
	function add(){ 
		...
		try{ 
			...
			$dataSession["notification"]	= $this->core_web_error->get_error($dataSession["user"]->userID);
			$dataSession["message"]			= $this->core_web_notification->get_message();
			$dataSession["head"]			= /*--inicio view*/ view('app_box_canceldocument/news_head',$dataView);//--finview
			$dataSession["body"]			= /*--inicio view*/ view('app_box_canceldocument/news_body',$dataView);//--finview
			$dataSession["script"]			= /*--inicio view*/ view('app_box_canceldocument/news_script',$dataView);//--finview
			$dataSession["footer"]			= "";
			return view("core_masterpage/default_masterpage",$dataSession);//--finview-r
	...
	function index($dataViewID = null){	
		try{ 
			...
			$objComponent		= $this->core_web_tools->getComponentIDBy_ComponentName("tb_transaction_master_cancel_invoice");
			if(!$objComponent)
			throw new \Exception("00409 EL COMPONENTE 'tb_transaction_master_cancel_invoice' NO EXISTE...");
			...
			$dataSession["notification"]	= $this->core_web_error->get_error($dataSession["user"]->userID);
			$dataSession["message"]			= $this->core_web_notification->get_message();
			$dataSession["head"]			= /*--inicio view*/ view('app_box_canceldocument/list_head');//--finview
			$dataSession["footer"]			= /*--inicio view*/ view('app_box_canceldocument/list_footer');//--finview
			$dataSession["body"]			= $dataViewRender; 
			$dataSession["script"]			= /*--inicio view*/ view('app_box_canceldocument/list_script');//--finview
	...
	function viewRegisterFormatoPaginaNormal(){
		try{ 
			...
			$datView["objStage"]					= $this->core_web_workflow->getWorkflowStage("tb_transaction_master_cancel_invoice","statusID",$datView["objTM"]->statusID,$companyID,$branchID,$roleID);
