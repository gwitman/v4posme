...
class Price_Model extends Model  {
   ...
   function get_rowByAll($companyID,$listPriceID){
		...
		$builder	= $db->table("tb_price");
		$sql = "";
		$sql = sprintf("select i.companyID,i.listPriceID,i.itemID,i.priceID,i.typePriceID,i.percentage,i.price,ci.name as tipoPrice,it.itemNumber,it.name as itemName,it.cost");
   ...
   function get_rowByPK($companyID,$listPriceID,$itemID,$typePriceID){
		...
		$builder	= $db->table("tb_price");    
		$sql = "";
		$sql = sprintf("select companyID,listPriceID,itemID,priceID,typePriceID,percentage,price");
		$sql = $sql.sprintf(" from tb_price i");		
		$sql = $sql.sprintf(" where i.companyID = $companyID");
		$sql = $sql.sprintf(" and i.listPriceID = $listPriceID");		
   ...
   function get_rowByItemID($companyID,$listPriceID,$itemID){
		...
		$builder	= $db->table("tb_price");    
		$sql = "";
		$sql = sprintf("select companyID,listPriceID,itemID,priceID,typePriceID,percentage,price,c.name as nameTypePrice");
		$sql = $sql.sprintf(" from tb_price i");		
