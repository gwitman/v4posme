<p style="margin-top:0pt; margin-bottom:8pt; text-align:center; line-height:108%; font-size:10pt;">
<img src="data:image/jpeg;base64,"
	 width="204"
	 height="120" 
/>

<p style="margin-top:0pt; margin-bottom:8pt; text-align:center; line-height:108%; font-size:10pt;">
	<span style="font-family:'Arial Black'; color:#0070c0;">FUNERIA UN ENCUENTRO CON DIOS</span>
</p>
<p style="margin-top:0pt; margin-bottom:8pt; text-align:center; line-height:108%; font-size:10pt;">
	<span style="font-family:'Arial Black'; color:#0070c0;">“PRESENTE EN LOS MOMENTOS DIFICILES 24/7”</span>
</p>
<p style="margin-top:0pt; margin-bottom:8pt; text-align:center; line-height:108%; font-size:10pt;">
	<span style="font-family:'Arial Black'; color:#0070c0;">DE LOS SEMAFOROS DE VILLA FLOR SUR 1 1/2 C ARRIBA M/I</span>
</p>
<p style="margin-top:0pt; margin-bottom:8pt; text-align:center; line-height:108%; font-size:10pt;">
	<span style="font-family:'Arial Black'; color:#0070c0;">RUC 0011910051026M</span>
</p>
<p style="margin-top:0pt; margin-bottom:8pt; text-align:center; line-height:108%; font-size:10pt;">
	<span style="font-family:'Arial Black'; color:#0070c0;">2253-5671/ 7797-0325</span>
</p>

<p style="margin-top:14pt; margin-bottom:8pt; text-align:center; line-height:normal; font-size:10pt;">
	<strong>
		<span style="font-family:'Arial Black'; color:#0070c0;">CONTRATO DE SERVICIO FUNERARIO</span>
	</strong>
</p>
<p style="margin-top:0pt; margin-bottom:8pt; text-align:center; line-height:108%; font-size:10pt;">
	<span style="font-family:'Arial Black';">&nbsp;</span>
</p>
<p style="margin-top:0pt; margin-bottom:8pt; text-align:center; line-height:108%; font-size:10pt;">
	<span style="font-family:'Arial Black';">&nbsp;</span>
</p>
<p style="margin-top:0pt; margin-bottom:8pt; line-height:108%; font-size:10pt;">
	<span style="font-family:'Arial Black';">GENERALIDADES DE EL COTRATANTE.</span>
</p>
<table style="width:100%" > 
	<tr>
		<td>
			<p style="margin-top:0pt;  text-align:justify; margin-bottom:0pt;  font-size:10pt;">
				<span style="font-family:'Arial Black';">NOMBRES Y DOS APELLIDOS</span>
			</p>
		</td>
		<td colspan="3">
			<?php echo $objFirstDetail["legalName"]; ?>
		</td>
	</tr>
	<tr>
		<td>
			<p style="margin-top:0pt;  text-align:justify; margin-bottom:0pt;  font-size:10pt;">
				<span style="font-family:'Arial Black';">DIRECCION DOMICILIAR</span>
			</p>
		</td>
		<td colspan="3">
			<?php echo $objFirstDetail["address"] == ""? "**NULL**" : $objFirstDetail["address"]; ?>
		</td>
	</tr>
	
	<tr>
		<td>
			<p style="margin-top:0pt;  text-align:justify; margin-bottom:0pt;  font-size:10pt;">
				<span style="font-family:'Arial Black';">FECHA DE NACIMIENTO</span>
			</p>
		</td>
		<td>
			<?php echo $objFirstDetail["birthDate"]; ?>
		</td>
		<td>
			<p style="margin-top:0pt;  text-align:justify; margin-bottom:0pt;  font-size:10pt;">
				<span style="font-family:'Arial Black';">EDAD</span>
			</p>
		</td>
		<td>
			<?php 
				$fech1 = new \DateTime( date('Y-m-d') );
				$fech2 = \DateTime::createFromFormat("Y-m-d",$objFirstDetail["birthDate"]);
				$result = $fech1->diff($fech2);
			?>
			<?php echo  $result->y  ; ?>
		</td>
	</tr>
	<tr>
		<td>
			<p style="margin-top:0pt;  text-align:justify; margin-bottom:0pt;  font-size:10pt;">
				<span style="font-family:'Arial Black';">CEDULA</span>
			</p>
		</td>
		<td>
			<?php echo $objFirstDetail["identification"]; ?>
		</td>
		<td>
			<p style="margin-top:0pt;  text-align:justify; margin-bottom:0pt;  font-size:10pt;">
				<span style="font-family:'Arial Black';">ESTADO CIVIL</span>
			</p>
		</td>
		<td>
			<?php echo $objFirstDetail["estadoCivil"]; ?>
		</td>
	</tr>
	<tr>
		<td>
			<p style="margin-top:0pt;  text-align:justify; margin-bottom:0pt;  font-size:10pt;">
				<span style="font-family:'Arial Black';">TELEFONO</span>
			</p>
		</td>
		<td>
			<?php echo $objFirstDetail["phoneNumber"]; ?>
		</td>
	</tr>
	<tr>
		<td>							
			<p style="margin-top:0pt;  text-align:justify; margin-bottom:0pt;  font-size:10pt;">
				<span style="font-family:'Arial Black';">PROFESION</span>
			</p>
		</td>
		<td>
			<?php echo $objFirstDetail["profesion"]; ?>
		</td>
		<td>							
			<p style="margin-top:0pt;  text-align:justify; margin-bottom:0pt;  font-size:10pt;">
				<span style="font-family:'Arial Black';">LUGAR DE TRABAJO</span>
			</p>
		</td>
		<td>
			<?php echo $objFirstDetail["lugarTrabajo"]; ?>
		</td>
	</tr>
<table/>



<p style="margin-top:0pt;  text-align:justify; margin-bottom:0pt;  font-size:10pt;">
	<span style="font-family:'Arial Black';">BENEFICIARIO EN CASO DE FALLECIMIENTO DE EL CONTRATANTE</span>
</p>
<p style="margin-top:0pt;  text-align:justify; margin-bottom:0pt;  font-size:10pt;">
	<span style="font-family:'Arial Black';">&nbsp;</span>
</p>
<table style="width:100%" >
	<tr>
		<td>
			<p style="margin-top:0pt;  text-align:justify; margin-bottom:0pt;  font-size:10pt;">
				<span style="font-family:'Arial Black';">NOMBRE Y APELLIDO</span>
			</p>
		</td>
		<td colspan="3">
			<?php echo $objFirstDetail["referenceClientName"]; ?>
		</td>
	</tr>
	<tr>
		<td>
			<p style="margin-top:0pt;  text-align:justify; margin-bottom:0pt;  font-size:10pt;">
				<span style="font-family:'Arial Black';">PARENTESCO</span>
			</p>
		</td>
		<td>
			<?php echo $objFirstDetail["Zona"]; ?>
		</td>
		<td>
			<p style="margin-top:0pt;  text-align:justify; margin-bottom:0pt;  font-size:10pt;">
				<span style="font-family:'Arial Black';">TELEFONO</span>
			</p>
		</td>
		<td>
			<?php echo $objFirstDetail["phoneNumberTransactionMaster"]; ?>
		</td>
	</tr>
	<tr>
		<td>
			<p style="margin-top:0pt;  text-align:justify; margin-bottom:8pt;  font-size:10pt;">
				<span style="font-family:'Arial Black';">CEDULA</span>
			</p>
		</td>
		<td colspan="3">
			<?php echo $objFirstDetail["referenceClientIdentifier"]; ?>
		</td>
	</tr>
</table>


<p style="margin-top:14pt; margin-bottom:14pt; line-height:normal; text-align:justify;  ">
	<span style="font-family:'Times New Roman';">FUNERARIA UN ENCUENTRO CON DIOS, constituida e inscrita conforme las leyes de la 
	Republica de Nicaragua, representada por ANGIE ARIEL BLANDON VANEGAS, con cedula de identidad n0 001-101005-1026M,  
	con domicilio EN VILLA FLOR SUR, DE LOS SEMAFOROS DE VILLA FLOR SUR 1 ½ C ARRIBA, en adelante denominada "La Funeraria".
	</span>
</p>

<p style="margin-top:14pt; margin-bottom:14pt; line-height:normal; text-align:justify;  ">
	<span style="font-family:'Times New Roman';">Y <?php echo $objFirstDetail["legalName"]; ?> con documento cedula de identidad número <?php echo $objFirstDetail["identification"]; ?> con domicilio en <?php echo $objFirstDetail["address"] == ""? "**NULL**" : $objFirstDetail["address"]; ?> en adelante denominado "El Contratante".</span>
</p>

<p style="margin-top:14pt; margin-bottom:14pt; line-height:normal;">
	<strong>
		<span style="font-family:'Times New Roman';">Ambas partes acuerdan lo siguiente:</span>
	</strong>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; line-height:normal;">
	<strong>
		<span style="font-family:'Times New Roman';">1. Objeto del Contrato</span>
	</strong>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; line-height:normal; text-align:justify;  ">
	<span style="font-family:'Times New Roman';">LA FUNERARIA se compromete a proporcionar los servicios funerarios detallados 
	en este contrato, a solicitud de EL CONTRATANTE, están de acuerdo en emitir y firmar el presente contrato de crédito de 
	Servicio Funerario, el cual está sujeto a las cláusulas que se enumeran y especifican a continuación:</span>
</p>
 
 
 
<p style="margin-top:14pt; margin-bottom:14pt; line-height:normal; font-size:12pt;">
	<strong>
		<span style="font-family:'Times New Roman';">2. Descripción del servicio adquirido por el contratante:</span>
	</strong>
</p>
<p style="margin-top:14pt; margin-bottom:10pt; line-height:normal;">
	<span style="font-family:'Times New Roman';">Contrato No <?php echo $objFirstDetail["documentNumber"]; ?></span>
	</br>
	<span style="font-family:'Times New Roman';">Estilo(s) (<?php echo $objFirstDetail["productNameLog"]; ?>)</span>	
	</br>
	<span style="font-family:'Times New Roman';">Numero de Servicios (<?php echo round($objFirstDetail["CantidadProductos"],1); ?>)
	</br>
	<span style="font-family:'Times New Roman';">El servicio funerario consta de <?php echo $objFirstDetail["Concepto"]; ?> </span>
</p>

<p style="margin-top:0pt; margin-bottom:8pt;">
	<strong>3. Vigencia:&nbsp;</strong>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; line-height:normal;">
	<span style="font-family:'Times New Roman';">Esta venta de crédito de servicio funerario entra en vigencia una vez que el 
	cliente haya cancelado al menos el 50 % del valor del contrato.</span>
</p>

<p style="margin-top:14pt; margin-bottom:14pt; line-height:normal; font-size:12pt;">
	<strong>
		<span style="font-family:'Times New Roman';">4. Valor Nominal:</span>
	</strong>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; text-align:justify; line-height:normal; font-size:12pt;">
	<span style="font-family:'Times New Roman'; font-size:11pt;">El o los servicios funerarios del presente contrato tiene un costo de: 
	<?php echo $objFirstDetail["cuota"]; ?> dolares pagaderos</span> <span style="font-family:'Times New Roman'; font-size:11pt;">&nbsp;&nbsp;
	</span> <span style="font-family:'Times New Roman'; font-size:11pt;">en cuotas <?php echo $objFirstDetail["period"]; ?> de 
	</span> <span style="font-family:'Times New Roman'; font-size:11pt;">&nbsp;&nbsp;&nbsp;</span> 
	<span style="font-family:'Times New Roman'; font-size:11pt;"><?php echo round($objFirstDetail["cuota"] / $objFirstDetail["term"],2) ; ?> 
	d&oacute;lares, o su equivalente en moneda nacional al tipo de cambio paralelo conforme al</span> 
	<span style="font-family:'Times New Roman'; font-size:11pt;">&nbsp;&nbsp;</span> 
	<span style="font-family:'Times New Roman'; font-size:11pt;">Sistema bancario nacional. El CONTRATANTE acuerda efectuar los pagos 
	los d&iacute;as <?php echo  \DateTime::createFromFormat("Y-m-d",$objFirstDetail["fechInicial"])->format("d")  ; ?>  de 
	cada mes</span> <span style="font-family:'Times New Roman';">.</span>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; line-height:normal; font-size:12pt;">
	<span style="font-family:'Times New Roman';">&nbsp;</span>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; line-height:normal; font-size:12pt;">
	<strong>
		<span style="font-family:'Times New Roman';">5. El pago de las cuotas:</span>
	</strong>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; text-align:justify; line-height:normal;">
	<span style="font-family:'Times New Roman';">El pago de las cuotas es anticipado y deber&aacute; ser 
	entregado en las oficinas de la FUNERARIA UN ENCUENTRO CON DIOS,</span>
	<span style="font-family:'Times New Roman';">&nbsp;&nbsp;</span>
	<span style="font-family:'Times New Roman';">quien a su vez podr&aacute; nombrar colector, sin que 
	ello exima al EL CONTRATANTE de la obligaci&oacute;n de tener sus cuotas al d&iacute;a para gozar 
	de los beneficios de esta venta al cr&eacute;dito. Para la cancelaci&oacute;n del valor 
	nominal arriba indicado las partes que suscriben el presente contrato que EL CONTRATANTE 
	entregar&aacute; <?php echo $objFirstDetail["term"]; ?> cuotas cada una con un valor de US$ 
	<?php echo round($objFirstDetail["cuota"] / $objFirstDetail["term"],2) ; ?> d&oacute;lares 
	de Estados Unidos de Am&eacute;rica o su equivalente en moneda nacional al tipo de cambio del 
	sistema bancario nacional.</span>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; line-height:normal;">
	<span style="font-family:'Times New Roman';">&nbsp;</span>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; line-height:normal; font-size:12pt;">
	<strong>
		<span style="font-family:'Times New Roman';">6.</span>
	</strong>
	<span style="font-size:11pt;">&nbsp;</span>
	<strong>
		<span style="font-family:'Times New Roman';">Entrega de servicio (s).</span>
	</strong>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; text-align:justify; line-height:normal;">
	<span style="font-family:'Times New Roman';">Para entrega del servicio o los servicios deber&aacute;</span>
	<span style="font-family:'Times New Roman';">&nbsp;&nbsp;</span>
	<span style="font-family:'Times New Roman';">EL CONTRATANTE haber cumplido con lo estipulado en la 
	cl&aacute;usula n&uacute;mero 4 del presente contrato y estar al d&iacute;a con el pago de sus cuotas.</span>
	<span style="font-family:'Times New Roman';">&nbsp;&nbsp;</span>
	<span style="font-family:'Times New Roman';">Cuando</span>
	<span style="font-family:'Times New Roman';">&nbsp;&nbsp;</span>
	<span style="font-family:'Times New Roman';">EL CONTRATANTE</span>
	<span style="font-family:'Times New Roman';">&nbsp;&nbsp;</span>
	<span style="font-family:'Times New Roman';">haga uso del o los servicios funerario (S), sin a&uacute;n haberlo (S)</span>
	<span style="font-family:'Times New Roman';">&nbsp;&nbsp;</span>
	<span style="font-family:'Times New Roman';">cancelado</span>
	<span style="font-family:'Times New Roman';">&nbsp;&nbsp;</span>
	<span style="font-family:'Times New Roman';">(S) FUNERARIA UN ENCUENTRO CON DIOS, reestructurar&aacute; el saldo 
	pendiente del servicio o servicios entregados para ser cancelados en un plazo comprendido entre uno (1) a 
	doce (12) meses.</span>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; line-height:normal;">
	<span style="font-family:'Times New Roman';">&nbsp;</span>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; line-height:normal; font-size:12pt;">
	<strong>
		<span style="font-family:'Times New Roman';">7. Pagare a la orden.</span>
	</strong>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; line-height:normal;">
	<span style="font-family:'Times New Roman';">Si el servicio funerario no ha sido cancelado al momento de ser 
	utilizado por EL CONTRATANTE este debe firmar un pagar&eacute; con un FIADOR por el equivalente del 
	monto pendiente para cancelar el servicio entregado.</span>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; text-align:justify; line-height:normal;">
	<span style="font-family:'Times New Roman';">El FIADOR debe ser una persona mayor de edad, con c&eacute;dula de 
	identidad y que demuestre con alg&uacute;n documento que tiene ingresos. La suma de los 
	ingresos de EL FIADOR debe ser igual o superior al monto estipulado en el pagar&eacute;.</span>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; text-align:justify; line-height:normal;">
	<span style="font-family:'Times New Roman';">&nbsp;</span>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; text-align:justify; line-height:normal; font-size:12pt;">
	<strong>
		<span style="font-family:'Times New Roman';">8. El Beneficiario.</span>
	</strong>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; text-align:justify; line-height:normal;">
	<span style="font-family:'Times New Roman';">El CONTRATANTE deber&aacute; nombrar un beneficiario mayor 
	de edad y sin l&iacute;mite de edad.</span>
	<span style="font-family:'Times New Roman';">&nbsp;&nbsp;</span>
	<span style="font-family:'Times New Roman';">Este beneficiario podr&aacute; reclamar el servicio 
	funerario siempre y cuando se haya cumplido</span>
	<span style="font-family:'Times New Roman';">&nbsp;&nbsp;</span>
	<span style="font-family:'Times New Roman';">la cl&aacute;usula n&uacute;mero 6 de este contrato.</span>
</p>

<p style="margin-top:14pt; margin-bottom:14pt; text-align:justify; line-height:normal; font-size:12pt;">
	<strong>
		<span style="font-family:'Times New Roman';">9. Excepci&oacute;n de obligaci&oacute;n.</span>
	</strong>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; text-align:justify; line-height:normal;">
	<span style="font-family:'Times New Roman';">La FUNERARIA UN ENCUENTRO CON DIOS</span>
	<span style="font-family:'Times New Roman';">&nbsp;&nbsp;</span>
	<span style="font-family:'Times New Roman';">est&aacute; obligado a prestar &eacute;l o los servicios 
	indicados en el presente contrato, pero cuando existan causas de caso fortuito o de fuerza mayor 
	quedar&aacute; exenta o dispensa temporalmente tal obligaci&oacute;n se extiende por caso fortuitos o 
	fuerza mayor guerras, asonada, rebeli&oacute;n, terremotos, erupciones volc&aacute;nicas, inundaciones 
	epidemias, pandemias, estados de sitio o estados de emergencia.</span>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; text-align:justify; line-height:normal;">
	<span style="font-family:'Times New Roman';">&nbsp;</span>
	<span style="font-family:'Times New Roman';">LA FUNERARIA se compromete a hacer lo humanamente posible 
	para cumplir sus obligaciones si las condiciones lo permiten.</span>
</p>

<p style="margin-top:14pt; margin-bottom:14pt; text-align:justify; line-height:normal; font-size:12pt;">
	<strong>
		<span style="font-family:'Times New Roman';">10. Anulaci&oacute;n.</span>
	</strong>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; text-align:justify; line-height:normal; font-size:12pt;">
	<span style="font-family:'Times New Roman'; font-size:11pt;">LA anulación del contrato de crédito de servicio funerario se 
	hará efectiva Si EL CONTRATANTE lo notifica por escrito o por acumular tres (3) cuotas consecutivas sin pagar.  
	Se acuerda que funeraria Un Encuentro con Dios 	no hará devolución de dinero bajo las condiciones ya especificadas 
	en esta misma cláusula.
	</span>	
</p>
<p style="margin-top:14pt; margin-bottom:14pt; text-align:justify; line-height:normal; font-size:12pt;">
	<strong>
		<span style="font-family:'Times New Roman';">11. Rehabilitaci&oacute;n</span>
	</strong>
	
</p>
<p style="margin-top:14pt; margin-bottom:14pt; text-align:justify; line-height:normal;">
	<span style="font-family:'Times New Roman';">EL CONTRATANTE</span>
	<span style="font-family:'Times New Roman';">&nbsp;&nbsp;</span>
	<span style="font-family:'Times New Roman';">podr&aacute; solicitar la rehabilitaci&oacute;n 
	del contrato de cr&eacute;dito de servicio funerario siempre y cuando cancele el valor de 
	las cuotas morosas. En todo caso FUNERARIA UN ENCUENTRO CON DIOS se reserva el derecho 
	de aprobar o negar la rehabilitaci&oacute;n del contrato.</span>
</p>

<p style="margin-top:14pt; margin-bottom:14pt; text-align:justify; line-height:normal; font-size:12pt;">
	<strong>
		<span style="font-family:'Times New Roman';">12. El &aacute;rea de cobertura.</span>
	</strong>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; text-align:justify; line-height:normal; font-size:12pt;">
	<span style="font-family:'Times New Roman';">&nbsp;</span>
	<span style="font-family:'Times New Roman'; font-size:11pt;">El &aacute;rea de cobertura 
	territorial que abarca el presente contrato es la ciudad de Managua si el 
	contratante desea para otra ciudad tendr&aacute; que Cancelar de inmediato 
	el valor de distancia seg&uacute;n lo indicado en la tabla existente de 
	funeraria un encuentro con Dios.</span>
</p>

<p style="margin-top:14pt; margin-bottom:14pt; text-align:justify; line-height:normal; font-size:12pt;">
	<strong>
		<span style="font-family:'Times New Roman';">13. Aceptaci&oacute;n y sometimiento:&nbsp;</span>
	</strong>
</p>
<p style="margin-top:14pt; margin-bottom:14pt; text-align:justify; line-height:normal;">
	<span style="font-family:'Times New Roman';">Funeraria un ENCUENTRO CON DIOS</span>
	<span style="font-family:'Times New Roman';">&nbsp;&nbsp;</span>
	<span style="font-family:'Times New Roman';">y EL CONTRATANTE: aceptan someterse a las cl&aacute;usulas 
	del presente contrato como la ley misma, debiendo aplicar las disposiciones legales 
	pertinentes todo lo no previsto y lo no resuelto por el mismo las partes contratantes 
	convienen en dilucidar cualquier controversia que pueda dar esta venta de cr&eacute;dito de 
	servicios funerarios en los tribunales de la capital de la Rep&uacute;blica de Nicaragua.</span>
</p>

<p style="margin-top:14pt; margin-bottom:14pt; text-align:justify; line-height:normal; font-size:12pt;">
	<strong>
		<span style="font-family:'Times New Roman';">Los contratantes habiendo le&iacute;do y aceptado 
		el estipulado en las cl&aacute;usulas anteriores enumeradas procedan a firmar 
		el presente contrato de cr&eacute;dito de servicio funerario emitido en dos 
		tantos en el mismo tenor en la ciudad de Managua a los d&iacute;as 
		<?php echo helper_DateToSpanish($objFirstDetail["fechActual"],"d"); ?>  del mes de 
		<?php echo helper_DateToSpanish($objFirstDetail["fechActual"],"F"); ?> del a&ntilde;o de 
		<?php echo helper_DateToSpanish($objFirstDetail["fechActual"],"Y"); ?>.</span>
	</strong>
</p>


<p style="margin-top:14pt; margin-bottom:14pt; text-align:center; line-height:normal; font-size:12pt;">
	<strong>
		<span style="font-family:'Times New Roman';">____________________________</span>
	</strong>
	<strong>
		<span style="font-family:'Times New Roman';">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
	</strong>
	<strong>
		<span style="font-family:'Times New Roman';">_____________________________</span>
	</strong>
</p>
<p style="margin-top:0pt; margin-bottom:8pt; text-align:center; line-height:108%; font-size:10pt;">
	<strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong>
	<strong>EL CONTRATANTE</strong>
	<strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	&nbsp;&nbsp;&nbsp;&nbsp;</strong>
	<strong>FUNERARIA UN ENCUENTRO CON DIOS</strong>
</p>