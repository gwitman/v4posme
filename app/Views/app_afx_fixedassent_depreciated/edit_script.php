				...
				<script>
					$(document).ready(function() {
						...
						$(document).on("click", "#btnAcept", function() {
							...
							$("#form-new-fixedasset_output").attr("action", "<?php echo base_url(); ?>/app_afx_fixedassent_depreciated/save/edit");
						...
						$(document).on("click", "#btnNewDetailTransaction", function() {
							...
							var url_request = "<?php echo base_url(); ?>/core_view/showviewbyname/<?php echo $objComponentFixedAsset->componentID; ?>/onCompleteFixedAsset/SELECCIONAR_FIXEDASSENT_DEPRECIATED/true/" + encodeURI('{\"currencyID\"|\"' + currencyID + '\"}') + "/false/not_redirect_when_empty";
						...
						$(document).on("click", "#btnDelete", function() {
							fnShowConfirm("Confirmar..", "Desea eliminar este Registro...", function() {
								...
								$.ajax({
									...
									dataType: 'json',
									type: 'POST',
									url: "<?php echo base_url(); ?>/app_afx_fixedassent_depreciated/delete",
									data: {
									...
									success: function(data) {
										...
										if (data.error) {
											...
											window.location = "<?php echo base_url(); ?>/app_afx_fixedassent_depreciated/index";
										}
									},
									error: function(xhr, data) {
										console.info("complete delete error");
						...
						$(document).on("click","#btnClickArchivo",function(){
							window.open("<?php echo base_url()."/core_elfinder/index/componentID/".$objComponentFixedAssetDepreciated->componentID."/componentItemID/".$objTM->transactionMasterID; ?>","blanck");
						});

						$(document).on("click", "#btnPrinter", function() {
							fnWaitOpen();
							window.open("<?php echo base_url(); ?>" + "/" + "/app_afx_fixedassent_depreciated/viewPrinterFormatoA4" + "/companyID/<?= $objTM->companyID; ?>/transactionID/<?= $objTM->transactionID; ?>/transactionMasterID/<?= $objTM->transactionMasterID; ?>", '_blank');
							fnWaitClose();
						});
					});
					...
					function validateForm() {
						...
						$("#body_tb_transaction_master_detail").each(function(index, obj) {
							...
							if (tableLength < 1) {
								fnShowNotification("Seleccione Un Activo a Depreciar", "error", timerNotification);
								result = false;
							}
						});

						//Validar que el ratio no sea cero, vacio o mayor que la liquidacion
						$("#body_tb_transaction_master_detail").find("tr").each(function(index, obj) {
							var ratio 			= parseFloat($(obj).find("#txtFixedAssetRatio").val()) 			|| 0;
							var settlement 		= parseFloat($(obj).find("#txtFixedSettlementAmount").val()) 	|| 0;
							var currentAmount 	= parseFloat($(obj).find("#txtFixedAssetCurrentAmount").val()) 	|| 0;
							...
							if(ratio > settlement)
							{
								fnShowNotification("El Ratio no puede ser mayor que la liquidacion", "error", timerNotification);
								result = false;	
								return false;
							}
							
							if(settlement < (ratio + currentAmount))
							{
								fnShowNotification("El Ratio mas el Monto Corriente no pueden ser mayor que la liquidacion ", "error", timerNotification);
								result = false;	
								return false;
							}
						});
