    ...
    $(document).ready(function() {
        ...
        var objDataEmployee         = JSON.parse('<?php echo json_encode($objEmployee) ?>');
        var objDataTM               = JSON.parse('<?php echo json_encode($objTM)?>');
        var objDataTMI              = JSON.parse('<?php echo json_encode($objTMI)?>');
        ...
        $("#btnClearEmployee").on("click", function() 
        {
            ...
            $("#txtTransactionID").val(objDataTM.transactionID);
            $("#txtTransactionMasterID").val(objDataTM.transactionMasterID);
        });
        ...
        $("#btnAcept").on("click", function()
        {
            if(validateForm())
            {
                ...
                .attr("method","POST")
                .attr("action","<?php echo base_url(); ?>/app_planilla_settlement/save/edit")
                .submit();
            }
        });
        ...
        function validateForm() 
        {
            ...
            inputsNumericos.forEach(input => {
                ...
                {
                    //Se eliminan las comas de los datos numericos en los inputs para evitar inconsistencias en la base de datos.
            ...
            if(isValidForm)
            {
                $('#txtTransactionOn').val(moment().format("YYYY-MM-DD HH:mm"));
            }
        ...
        function onUpdatedEmployeeData(selectedEmployeeData)
        {
            ...
            {
                EmployeeSalary          = objDataEmployee.hourCost;
                EmployeeStartDate       = objDataEmployee.startOn;
                // EmployeeEndDate         = objDataEmployee.endOn;
                EmployeeVacationBalance = objDataEmployee.vacationBalanceDay || 0;
                EmployeeAmountSaving    = objDataEmployee.amountSaving || 0;
            }
            else
            {   
            ...
            let EmployeeEndDate             = $("#txtEmployeeEndDate").val();
            ...
            totalIngresoFinal   = parseFloat(totalSalarioProporcional) + parseFloat(totalAhorroAcumulado) + parseFloat(totalComision) + parseFloat(totalBono) + parseFloat(totalPagoVacacionesAcumuladas) + parseFloat(totalPagoMesTrece) + parseFloat(totalPagoAntiguedad);
            
            //Calculo total de Deducciones
            totalDeduccionFianl = parseFloat(totalINSS) + parseFloat(totalIR) + parseFloat(totalIRVacaciones) + parseFloat(totalPrestamos) + parseFloat(totalLLegadasTarde) + parseFloat(totalAdelantos);
