	...
	$(document).ready(function(){	
		...
		$(document).on("click","#btnEdit",function(){
			...
			if(objRowTableListView != undefined){
				...
				fnWaitOpen();				
				window.location	= "<?php echo base_url(); ?>/app_purchase_purchaseorden/edit/companyID/"+data[0]+"/transactionID/"+data[1]+"/transactionMasterID/"+data[2];
			}
			else{
		...
		$(document).on("click","#btnSearchItem",function(){
					...
					$.ajax({									
						...
						dataType    : 'json',
						type        : 'POST',
						url  		: "<?php echo base_url(); ?>/app_purchase_purchaseorden/searchTransactionMaster",
						...
						success:function(data){
							...
							if(data.error){
								...
								window.location = "<?php echo base_url(); ?>/app_purchase_purchaseorden/edit/companyID/"+data.companyID+"/transactionID/"+data.transactionID+"/transactionMasterID/"+data.transactionMasterID;
							}
						},
		...
		$(document).on("click","#btnNuevo",function(){
			fnWaitOpen();
			window.location	= "<?php echo base_url(); ?>/app_purchase_purchaseorden/add";
		});
	});
	
	function fn_aceptCallback(data){
			var dataViewID 	= data[0];
			window.location = "../../app_purchase_purchaseorden/index/"+dataViewID;   
	}					
</script>