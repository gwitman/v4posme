	...
	$(document).ready(function(){	
		...
		$(document).on("click","#btnEdit",function(){
			...
			if(objRowTableListView != undefined){
				...
				window.location	= "<?php echo base_url(); ?>/app_box_provisioned/edit/companyID/"+data[0]+"/transactionID/"+data[1]+"/transactionMasterID/"+data[2];
			}
			else{
		...
		$(document).on("click","#btnSearchTransaction",function(){
					...
					$.ajax({									
						...
						dataType    : 'json',
						type        : 'POST',
						url  		: "<?php echo base_url(); ?>/app_box_provisioned/searchTransactionMaster",
						...
						success:function(data){
							...
							if(data.error){
								...
								window.location = "<?php echo base_url(); ?>/app_box_provisioned/edit/companyID/"+data.companyID+"/transactionID/"+data.transactionID+"/transactionMasterID/"+data.transactionMasterID;
							}
						},
		...
		$(document).on("click","#btnNuevo",function(){
			fnWaitOpen();
			window.location	= "<?php echo base_url(); ?>/app_box_provisioned/add";
		});
	});
