	...
	$(document).ready(function(){		
		...
		$(document).on("click","#btnEdit",function(){
			...
			if(objRowTableListView != undefined){
				...
				window.location	= "<?php echo base_url(); ?>/app_branch/edit/companyID/"+data[0]+"/branchID/"+data[1];
			}
			else{
			...
			if(objRowTableListView != undefined){
				...
				fnShowConfirm("Confirmar..","Desea eliminar este Registro...",function(){
					...
					$.ajax({									
						...
						data 		: {companyID : data[0], branchID :data[1]},
		...
		$(document).on("click","#btnNuevo",function(){
			fnWaitOpen();
			window.location	= "<?php echo base_url(); ?>/app_branch/add";
		});
	});
	
	function fn_aceptCallback(data){
			var dataViewID 	= data[0];
			window.location = "../../app_branch/index/"+dataViewID;   
	}					
</script>