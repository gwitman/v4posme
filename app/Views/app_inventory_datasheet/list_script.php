	...
	$(document).ready(function(){	
		...
		$(document).on("click","#btnEdit",function(){
			...
			if(objRowTableListView != undefined){
				...
				fnWaitOpen();				
				window.location	= "<?php echo base_url(); ?>/app_inventory_datasheet/edit/companyID/"+data[0]+"/itemDataSheetID/"+data[1];
			}
			else{
		...
		$(document).on("click","#btnSearchItem",function(){
					...
					$.ajax({									
						...
						dataType    : 'json',
						type        : 'POST',
						url  		: "<?php echo base_url(); ?>/app_inventory_datasheet/searchItem",
		...
		$(document).on("click","#btnNuevo",function(){
			fnWaitOpen();
			window.location	= "<?php echo base_url(); ?>/app_inventory_datasheet/add";
		});
	});
	
	function fn_aceptCallback(data){
			var dataViewID 	= data[0];
			window.location = "../../app_inventory_datasheet/index/"+dataViewID;   
	}					
</script>