	...
	$(document).ready(function(){	
		...
		$(document).on("click","#btnEdit",function(){
			...
			if(objRowTableListView != undefined){
				...
				window.location	= "<?php echo base_url(); ?>/app_afx_fixedassent/edit/companyID/"+data[0]+"/branchID/"+data[1]+"/fixedAssentID/"+data[2];
			}
			else{
			...
			if(objRowTableListView != undefined){
				...
				fnShowConfirm("Confirmar..","Desea eliminar este Registro...",function(){
					...
					$.ajax({									
						...
						data 		: {companyID : data[0], branchID :data[1],fixedAssentID: data[2] },
		...
		$(document).on("click","#btnNuevo",function(){
			fnWaitOpen();
			window.location	= "<?php echo base_url(); ?>/app_afx_fixedassent/add";
		});
	});
	
	function fn_aceptCallback(data){
			var dataViewID 	= data[0];
			window.location = "../../app_afx_fixedassent/index/"+dataViewID;   
	}					
</script>