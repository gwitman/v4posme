	...
	$(document).ready(function(){	
		...
		$(document).on("click","#btnEdit",function(){
			...
			if(objRowTableListView != undefined){
				...
				window.location	= "<?php echo base_url(); ?>/app_accounting_journal/edit/companyID/"+data[0]+"/journalEntryID/"+data[1];
			}
			else{
		...
		$(document).on("click","#btnSearchJournal",function(){
					...
					$.ajax({									
						...
						dataType    : 'json',
						type        : 'POST',
						url  		: "<?php echo base_url(); ?>/app_accounting_journal/searchJournal",
						...
						success:function(data){
							...
							if(data.error){
								...
								window.location = "<?php echo base_url(); ?>/app_accounting_journal/edit/companyID/"+data.companyID+"/journalEntryID/"+data.journalEntryID;
							}
						},
		...
		$(document).on("click","#btnNuevo",function(){
			fnWaitOpen();
			window.location	= "<?php echo base_url(); ?>/app_accounting_journal/add";
		});
	});
