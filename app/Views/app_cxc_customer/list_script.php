	...
	$(document).ready(function(){	
		...
		$(document).on("click","#btnEdit",function(){
			...
			if(objRowTableListView != undefined){
				...
				window.location	= "<?php echo base_url(); ?>/app_cxc_customer/edit/companyID/"+data[0]+"/branchID/"+data[1]+"/entityID/"+data[2];
			}
			else{
		...
		$(document).on("click","#btnNuevo",function(){
			fnWaitOpen();
			window.location	= "<?php echo base_url(); ?>/app_cxc_customer/add";
		});
	});
	
	function fn_aceptCallback(data){
			var dataViewID 	= data[0];
			window.location = "../../app_cxc_customer/index/"+dataViewID;   
	}					
</script>