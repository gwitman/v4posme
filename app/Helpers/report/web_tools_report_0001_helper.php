...
function helper_reporteA4TransactionMasterOutherOutputA4Generic(
    $titulo,
    $objCompany,
    $objParameterLogo,
    $objTransactionMastser,
...
{
	 // En lugar de $this->core_web_parameter->..., instancias la librería directamente:
...
function helper_reporteA4TransactionMasterOutherOutputGeneric(
    $titulo,
    $objCompany,
    $objParameterLogo,
    $objTransactionMastser,
...
)
{
    ...
    $numberDocument =  str_replace("OSS", "Ajuste de Salida ", strtoupper($objTransactionMastser->transactionNumber) ); ;
	$tipoDocumento  = "SALIDA DE INVENTARIO";
	$currencyName	= $objTransactionMastser->currencyID == 1 ? "C$" : "$";
	...
	$f_html = $f_html."
			<table style='width:98%' > 
				<tr style='background-color: #ccc;'>
					<td style='text-align:center;width:70px; font-weight: bold;"."' >ARTICULO</td>
					<td style='text-align:center; font-weight: bold;"."' >DESCRIPCION</td>
					<td style='text-align:center;width:70px; font-weight: bold;"."' >Cost. Unit.</td>
					<td style='text-align:center;width:70px; font-weight: bold;"."' >Cant.</td>
					<td style='text-align:center;width:70px; font-weight: bold;"."' >Descuento</td>
					<td style='text-align:center;width:70px; font-weight: bold;"."' >TOTAL</td>
				<tr>
	...
	for($i = 0 ; $i < 22 ; $i++)
	{
		...
		{
			$cantidad++;
			$subTotal	= $subTotal + (round($objDetail[$i]->unitaryCost,2)  * round($objDetail[$i]->quantity,2));
			$f_html 	= $f_html."			
				<tr>
					<td style='text-align:left;width:70px' >".$objDetail[$i]->itemNumber."</td>
					<td style='text-align:left;' >".$objDetail[$i]->itemName."</td>
					<td style='text-align:center;width:70px' >".$currencyName." ".number_format(round($objDetail[$i]->unitaryCost,2),2,".",",")."</td>
					<td style='text-align:center;width:70px' >".number_format(round($objDetail[$i]->quantity,2),2,".",",")."</td>
					<td style='text-align:center;width:70px' >".$currencyName." 0.00</td>
					<td style='text-align:center;width:70px' >".$currencyName." ".number_format(round($objDetail[$i]->unitaryCost,2)  * round($objDetail[$i]->quantity,2)   ,2,".",",")."</td>
				<tr>
			";
		}
		else
						...
						<tr>
							...
							<td  style='text-align:left;vertical-align:top;widht:100px;' >
								<table style='width:100%;'>
									<tr>										
										<td style='width:70px'>Cantidad</td>
										<td style='text-align:right;width:70px'>".$currencyName." ".number_format ( round($subTotal,2) , 2 , ".","," ) ."</td>
									</tr>
									<tr>
										<td><!--Descuentos--></td>
										<td style='text-align:right'><!--0.00--></td>
			...
			<tr>
				...
				<td>
					...
					<td  style='text-align:left;vertical-align:top;widht:auto;font-weight: bold;' >              
						Monto a cancelar: ".$objCurrency->simbol." ".number_format($objTransactionMastser->amount,2,".",",")."
					</td>                 
					</tr>		  
					</table>
				</td>
...
function helper_reporte80mmTransactionMasterMiniShalom(
    $titulo,
    $objCompany,
    $objParameterLogo,
    $objTransactionMastser,
            ...
            foreach ($row as $col => $key){				
				...
				//datos
					...
					{	
						...
						if($rowin > 0 && $colun == 0 && !is_numeric($texto) )
						{
							$cuerpo 	= $cuerpo."<tr style='margin:0;padding:0;' >";															
							$cuerpo = $cuerpo."<td colspan='3' style='margin:0;padding:0;' >";
							$cuerpo = $cuerpo.$confiDetalle[$colun]["prefix_row_data"]." ".$texto;
							$cuerpo = $cuerpo."</td>";								
							$cuerpo = $cuerpo."</tr >";
						}
						else 
						{
							...
							{
								$cuerpo = $cuerpo."<tr style='margin:0;padding:0;' >";		
								$cuerpo = $cuerpo."<td colspan='1' style='text-align:left;margin:0;padding:0;'  >".$confiDetalle[$colun]["prefix_row_data"]." ".$texto."</td>";								
							}
							
							if($colun == 1)
							{
								$cuerpo = $cuerpo."<td colspan='1' style='text-align:right;margin:0;padding:0;' >".$confiDetalle[$colun]["prefix_row_data"]." ".$texto."</td>";		
							}
...
function helper_reporte80mmTransactionMasterBranch(
    $titulo,
    $objCompany,
    $objParameterLogo,
    $objTransactionMastser,
...
function helper_reporte80mmTransactionMasterPasteleriaLizzette(
    $titulo,
    $objCompany,
    $objParameterLogo,
    $objTransactionMastser,
...
)
{
    ...
    $html 		= str_replace("[[DETALLE]]", $cuerpo, $html);
	$htmlcopy 	= $html;
	$html       = str_replace("[[FORMATO_DOCUMENTO]]","ORIGINAL",$html);
	$htmlcopy 	= str_replace("[[FORMATO_DOCUMENTO]]","COPIA",$htmlcopy);	
	$htmlPdf	= $htmlPdf.$html."<div style='page-break-before:always;' ></div>".$htmlcopy."</body></html>";
	
    return $htmlPdf;
}

function helper_reporte80mmTransactionMasterBoxShareRepuestoCristoRey(
    $titulo,
    $objCompany,
    $objParameterLogo,
    $objTransactionMastser,
...
)
{
    ...
    $html    = "
                    ...
                    <body>
                      ...
                      <table style='width:100%'>
                        <tr>
                          <td colspan='3' style='text-align:center'>
                            <img  src='".$base64."' width='140' height='130'   alt='Logotipo' />
                          </td>
                        </tr>
    ...
    $html	= $html."
						...
						<tr>
						  <td colspan='3'>							
                            ". strtoupper( $objTransactionMasterInfo->referenceClientName == "" ?   $objEntidadNatural->firstName." ".$objEntidadNatural->lastName  :  $objTransactionMasterInfo->referenceClientName)  ."
                          </td>
                        </tr>
...
)
{
    ...
    $html    		= "
                    ...
                    <head>
					  ...
					  <style>
						...
						body {
							...
							background-size: auto; /* ajustar imagen al tamaño */
						...
						#firma 
						{
							background-image: url(". $selloBase64.");
							background-size: 200px 150px; /* ancho y alto de la imagen */
			...
			$html	= $html."		
                        ...
                        <table id='firma' style='width: 400px; margin: 0 auto; text-align: center;'>
							<tr><td style='text-align:left;'>&nbsp;</td></tr>	
							<tr><td style='text-align:left;'>&nbsp;</td></tr>	
							<tr><td style='text-align:left;'>&nbsp;</td></tr>	
							<tr><td style='text-align:left;'>&nbsp;</td></tr>	
...
function helper_reporteGeneralCreateTableGroupBy($objDetail,$configColumn,$widht,$titulo = NULL,$backgournd = NULL,$color = NULL){
	...
	foreach($objDetail as $i){
		...
		foreach($configColumn as $key => $value ){
			...
			{							
					...
					$table = $table. "<td nowrap style='text-overflow:ellipsis;overflow:hidden;white-space:nowrap;text-align:".$value["Alineacion"].";".$value["Style"]." ' colspan='".$value['Colspan']."'  class='border'>";										
					$valueField 			= $value["FiledSouce"] == "" ? "" : ($i[$value["FiledSouce"] ] );
					//$valueField 			= $value["FiledSouce"] == "" ? "" : ($i[$value["FiledSouce"] ]." - ".$autoIncrementValue++ );
					$tipoData				= array_key_exists("Formato",$value) ? $value["Formato"] : "" ;
					$sumaryzar				= array_key_exists("Total",$value) ? $value["Total"] : False ; 
					$prefix					= array_key_exists("FiledSoucePrefix",$value) ? $value["FiledSoucePrefix"] : "" ;
					$FiledSoucePrefixCustom	= array_key_exists("FiledSoucePrefixCustom",$value) ? $value["FiledSoucePrefixCustom"] : "" ;										
...
function helper_reporte80mmInventoryInputGenerico(
    $titulo,
    $objCompany,
    $objParameterLogo,
    $objTM,
...
function helper_reporte80mmInventoryOutputGenerico(
    $titulo,
    $objCompany,
    $objParameterLogo,
    $objTM,
