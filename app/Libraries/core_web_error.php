...
class core_web_error {
   ...
   function get_error($userID){
		...
		$objListError		= $Error_Model->get_rowByUser($userID);
		$objListErrorCount	= $Error_Model->get_rowByUserCount($userID);
		...
		foreach($objListError as $i){
			$counter++;
			$item	.= '<li role="presentation"><a href="'.site_url().'/core_notification/index" class=""><i class="icon16 i-bell-2"></i> '.$i->notificated.' </a></li>';
		}
		
		if($objListErrorCount > $counter )
		$item	.= '<li role="presentation"><a href="'.site_url().'core_notification/index" class=""><i class="icon16 i-bell-2"></i> ......... </a></li>';
