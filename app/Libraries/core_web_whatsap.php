...
class core_web_whatsap {
   ...
   )
   {
        ...
        //wg-return $this->response->setJSON([
        //wg-    'success' => true,
        //wg-    'response' => json_decode($response, true)
        //wg-]);
   ...
   function sendMessageZAPIioImage( $companyID,$urlImagen, $message, $phoneDestino)
   {
	...
	function sendMessageUltramsgOnlyTextMasive($companyID,$chatSend,$pathRemember) 
	{
		...
		foreach ($chatSend as $customer) {
			...
			$data = [
				"to"        		=> clearNumero($customer["phoneNumber"]),
	...
	function sendMessageByEvolutionApiImage($companyID, $urlImagen, $message, $phoneDestino, $instanciaName)
	{
		...
		$url 		= rtrim($objCP_WhatsapUrlSendMessage->value, '/') . '/message/sendMedia/' . $objPWhatsappInstanciName->value;
		$payload 	= [
			'number'    => $phoneDestino,
			'mediatype' => 'image',
			'media'     => $urlImagen,
	...
	function sendMessageByEvolutionApiAudio($companyID, $urlAudio, $phoneDestino, $instanciaName)
	{
		...
		$url 		= rtrim($objCP_WhatsapUrlSendMessage->value, '/') . '/message/sendWhatsAppAudio/' . $objPWhatsappInstanciName->value;
		$payload 	= [
			'number' => $phoneDestino,
			'audio'  => $urlAudio
		];
